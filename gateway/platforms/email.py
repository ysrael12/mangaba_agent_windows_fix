"""
Email platform adapter for the Mangaba gateway.

Allows users to interact with Mangaba by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "mangaba-agent" "version" "{_mangaba_version}" '
            '"vendor" "dheiver2" '
            '"support-email" "noreply@dheiver2.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Mangaba Agent")
        ...
        msg_id = f"<mangaba-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
