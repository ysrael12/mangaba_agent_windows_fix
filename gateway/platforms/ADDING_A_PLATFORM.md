...
There are two ways to add a platform to the Mangaba gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.mangaba/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Mangaba code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `mangaba gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `mangaba_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Mangaba core.
...
And add it to the `mangaba-gateway` composite:

```python
"mangaba-gateway": {
    "includes": [..., "mangaba-your-platform"]
}
```
...
## 12. Status Display (`mangaba_cli/status.py`)
...
## 13. Gateway Setup Wizard (`mangaba_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ mangaba_cli/ toolsets.py \
  --include="*.py" -l | sort -u
