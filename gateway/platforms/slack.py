...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_mangaba_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /mangaba prefix). A single regex
            ...
            # manifest (see `mangaba slack manifest`). In Socket Mode, Slack
            ...
            from mangaba_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/mangaba$")

            @self._app.command(_slash_pattern)
            async def handle_mangaba_command(ack, command):
            ...
            for _action_id in (
                "mangaba_approve_once",
                "mangaba_approve_session",
                "mangaba_approve_always",
                "mangaba_deny",
            ):
            ...
            for _action_id in (
                "mangaba_confirm_once",
                "mangaba_confirm_always",
                "mangaba_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Mangaba handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Mangaba session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from mangaba_cli.commands import is_gateway_known_command
                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@mangaba`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "mangaba_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "mangaba_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "mangaba_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "mangaba_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "mangaba_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "mangaba_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "mangaba_confirm_once": "once",
            "mangaba_confirm_always": "always",
            "mangaba_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "mangaba_approve_once": "once",
            "mangaba_approve_session": "session",
            "mangaba_approve_always": "always",
            "mangaba_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/mangaba <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/mangaba
        ...
        if slash_name in {"mangaba", ""}:
            # Legacy /mangaba <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from mangaba_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/mangaba   ``).
        ...
        # "Running /cmd…" ack shown by handle_mangaba_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/mangaba <question>" must produce public replies so
