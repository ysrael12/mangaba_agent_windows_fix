...
# Audio file extensions Mangaba recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Mangaba' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Mangaba-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.mangaba/.env manually."
)
...
# Default location: {MANGABA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_mangaba_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_mangaba_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_mangaba_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_mangaba_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_mangaba_dir("cache/screenshots", "browser_screenshots")
_MANGABA_HOME = get_mangaba_home()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "MANGABA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _MANGABA_HOME / "image_cache",
    _MANGABA_HOME / "audio_cache",
    _MANGABA_HOME / "video_cache",
    _MANGABA_HOME / "document_cache",
    _MANGABA_HOME / "browser_screenshots",
)
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under Mangaba-managed media caches, or roots the
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?mangaba\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+mangaba[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from mangaba_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          MANGABA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          MANGABA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          MANGABA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("MANGABA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("MANGABA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("MANGABA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _mangaba_run_generation is set on
