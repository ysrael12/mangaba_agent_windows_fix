...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/dheiver2/mangaba-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    mangaba_meta = (fm.get("metadata") or {}).get("mangaba") or {}
    tags = mangaba_meta.get("tags") or []
    related = mangaba_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `mangaba skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Mangaba Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Mangaba ships with a large built-in skill library copied into `~/.mangaba/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Mangaba also syncs bundled skills on `mangaba update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.mangaba/skills/` tree, it is still shipped with Mangaba; restore it with `mangaba skills reset <name> --restore`.",
        "",
