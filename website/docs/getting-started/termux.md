...
description: "Run Mangaba Agent directly on an Android phone with Termux"
---

# Mangaba on Android with Termux

This is the tested path for running Mangaba Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Mangaba from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Mangaba now ships a Termux-aware installer path:
...
- links `mangaba` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `mangaba` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/mangaba" "$PREFIX/bin/mangaba"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `mangaba` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.mangaba/.env`.
...
### `mangaba doctor` says ripgrep or Node is missing
