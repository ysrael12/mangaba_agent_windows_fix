...
Mangaba Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Mangaba checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.mangaba/config.yaml`:
...
1. **CLI flag**: Start a session with `mangaba --yolo` or `mangaba chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `MANGABA_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `MANGABA_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Mangaba shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Mangaba refuses to run them **regardless** of:
...
Configure the timeout in `~/.mangaba/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.mangaba/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.mangaba/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` mangaba/gateway | Self-termination prevention |
...
The `MANGABA_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.mangaba/config.yaml`:
...
:::tip
Use `mangaba config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Mangaba controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.mangaba/.env`:
...
For more flexible authorization, Mangaba includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `mangaba pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.mangaba/config.yaml`:
...
mangaba pairing list

# Approve a pairing code
mangaba pairing approve telegram ABC12DEF

# Revoke a user's access
mangaba pairing revoke telegram 123456789

# Clear all pending codes
mangaba pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.mangaba/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Mangaba applies strict security hardening to every container.
...
Container resources are configurable in `~/.mangaba/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.mangaba/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `MANGABA_HOME`. Skills declare these in frontmatter:
...
When loaded, Mangaba checks if these files exist in the active profile's `MANGABA_HOME` and registers them for mounting:
...
Paths are relative to `~/.mangaba/`. Files are mounted to `/root/.mangaba/` inside the container.
...
| **terminal** (local) | Blocks explicit Mangaba infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Mangaba infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Mangaba integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Mangaba under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.mangaba/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.mangaba/logs/` for unauthorized access attempts
10. **Keep updated** — run `mangaba update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.mangaba/.env`:
...
TERMINAL_SSH_USER=mangaba
TERMINAL_SSH_KEY=~/.ssh/mangaba_agent_key
```
...
Mangaba ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `mangaba_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `mangaba doctor` for the full remediation.
- **`mangaba doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
mangaba doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Mangaba installs these **lazily** on first use rather than eagerly under `mangaba-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `mangaba tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `mangaba tools`.
