...
- Give it a name like "mangaba-agent"
- Select scopes:
...
- Give it a title like "mangaba-agent-&lt;machine-name>"
...
elif [ -f ~/.mangaba/.env ] && grep -q "^GITHUB_TOKEN=" ~/.mangaba/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.mangaba/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
