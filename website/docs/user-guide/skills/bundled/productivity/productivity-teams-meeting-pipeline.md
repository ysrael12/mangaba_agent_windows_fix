...
description: "Operate the Teams meeting summary pipeline via Mangaba CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Mangaba CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `mangaba teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.mangaba/.env`:
...
```bash
mangaba teams-pipeline validate              # config snapshot — run first after any change
mangaba teams-pipeline token-health          # Graph token status
mangaba teams-pipeline token-health --force-refresh   # force a fresh token acquisition
mangaba teams-pipeline list                  # recent meeting jobs
mangaba teams-pipeline list --status failed  # only failed jobs
mangaba teams-pipeline show <job-id>         # full detail of one job
mangaba teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
mangaba teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
mangaba teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
mangaba teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
mangaba teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

mangaba teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
mangaba teams-pipeline delete-subscription <sub-id>
mangaba teams-pipeline maintain-subscriptions            # renew near-expiry ones
mangaba teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `mangaba teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `mangaba cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
