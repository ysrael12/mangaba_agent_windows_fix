...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Mangaba Agent |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Mangaba' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
