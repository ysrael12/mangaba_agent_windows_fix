...
description: "Configure and use Honcho memory with Mangaba -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Mangaba -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Mangaba

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Mangaba profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/mangaba#running-honcho-locally-with-mangaba

### Verify

```bash
mangaba honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Mangaba profile's AI peer
...
Honcho models conversations as interactions between **peers**. Mangaba creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Mangaba instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Mangaba run |
| `global` | Single session across all directories |

Manual override: `mangaba honcho map my-project-name`
...
Each Mangaba profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block mangaba.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `mangaba.coder` host block in `honcho.json`
...
```bash
mangaba honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Mangaba when Honcho memory is active.
...
Config file: `$MANGABA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `mangaba honcho setup`. Ensure `memory.provider: honcho` is in `~/.mangaba/config.yaml`.

### Memory not persisting across sessions
Check `mangaba honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `mangaba profile create <name> --clone`. For existing profiles: `mangaba honcho sync`.
...
| Command | Description |
|---------|-------------|
| `mangaba honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `mangaba honcho status` | Show resolved config, connection test, peer info for active profile |
| `mangaba honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `mangaba honcho disable` | Disable Honcho for the active profile |
| `mangaba honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `mangaba honcho peers` | Show peer identities across all profiles |
| `mangaba honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `mangaba honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `mangaba honcho sessions` | List known directory-to-session-name mappings |
| `mangaba honcho map <name>` | Map current working directory to a Honcho session name |
| `mangaba honcho identity` | Seed AI peer identity or show both peer representations |
| `mangaba honcho sync` | Create host blocks for all Mangaba profiles that don't have one yet |
| `mangaba honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Mangaba + Honcho |
| `mangaba memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `mangaba memory status` | Show active memory provider and config |
| `mangaba memory off` | Disable external memory provider |
