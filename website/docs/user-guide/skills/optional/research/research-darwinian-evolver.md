...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Mangaba Agent |
| License | MIT |
...
import upstream classes into Mangaba itself.
...
```bash
mkdir -p ~/.mangaba/cache/darwinian-evolver && cd ~/.mangaba/cache/darwinian-evolver
...
```bash
cd ~/.mangaba/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.mangaba/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.mangaba/skills/research/darwinian-evolver
DE_DIR=~/.mangaba/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Mangaba core.
   Custom driver scripts under `~/.mangaba/skills/...` are user-side and fine.
