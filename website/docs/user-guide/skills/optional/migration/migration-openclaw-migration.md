---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Mangaba Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Mangaba Agent"
---
...
Migrate a user's OpenClaw customization footprint into Mangaba Agent. Imports Mangaba-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Mangaba Agent (Dheiver Santos) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Mangaba`, `Memory`, `Persona`, `Import` |
| Related skills | [`mangaba-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-mangaba-agent) |
...
# OpenClaw -> Mangaba Migration

Use this skill when a user wants to move their OpenClaw setup into Mangaba Agent with minimal manual cleanup.
...
```bash
mangaba claw migrate              # Full interactive migration
mangaba claw migrate --dry-run    # Preview what would be migrated
mangaba claw migrate --preset user-data   # Migrate without secrets
mangaba claw migrate --overwrite  # Overwrite existing conflicts
mangaba claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `mangaba setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_mangaba.py` to:

- import `SOUL.md` into the Mangaba home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Mangaba memory entries
- merge OpenClaw command approval patterns into Mangaba `command_allowlist`
- migrate Mangaba-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.mangaba/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Mangaba workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.mangaba/tts/`
- archive non-secret docs that do not have a direct Mangaba destination
...
- `scripts/openclaw_to_mangaba.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py`

Do not guess a shorter path like `~/.mangaba/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.mangaba/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Mangaba-compatible secrets, currently:
...
Mangaba CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Mangaba. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Mangaba workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Mangaba skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.mangaba/skills/migration/openclaw-migration/scripts/openclaw_to_mangaba.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Mangaba unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Mangaba targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Mangaba destination. Unsupported auth blobs must still be reported as skipped.
...
- Mangaba persona state imported
- Mangaba memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.mangaba/skills/openclaw-imports/`
