...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Mangaba Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Mangaba terminal calls (desktop app flow only)
...
### Service Account (recommended for Mangaba)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.mangaba/.env` (the skill will prompt for this on first load).
...
## Mangaba Execution Pattern (desktop app flow)

Mangaba terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/mangaba-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/mangaba-op.sock"
