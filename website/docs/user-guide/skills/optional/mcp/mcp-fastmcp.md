...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Mangaba or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Mangaba. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.mangaba/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.mangaba/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Mangaba integration, either:

- configure the server in `~/.mangaba/config.yaml` using the `native-mcp` skill, or
...
### Mangaba cannot see the deployed server

The server-building part may be correct while the Mangaba config is not. Load the `native-mcp` skill and configure the server in `~/.mangaba/config.yaml`, then restart Mangaba.
