...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${MANGABA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MANGABA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MANGABA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client mangaba
```
...
```bash
PUBLISH="${MANGABA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client mangaba
```
...
```bash
DRIVE="${MANGABA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `mangaba`)    |
