...
| | |
|---|---|
| Source | Optional — install with `mangaba skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Mangaba Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.mangaba/.env`:
...
```bash
CANVAS="python $MANGABA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
