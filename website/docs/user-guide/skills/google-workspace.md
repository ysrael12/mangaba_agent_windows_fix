...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Mangaba. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/nicholasgasior/gws) when available for broader coverage, and falls back to Google's Python client libraries otherwise.

**Skill path:** `skills/productivity/google-workspace/`

## Setup

The setup is fully agent-driven — ask Mangaba to set up Google Workspace and it walks you through each step. The flow:
...
3. **Authorize** — Mangaba generates an auth URL, you approve in the browser, paste back the redirect URL
...
| Problem | Fix |
|---------|-----|
| `NOT_AUTHENTICATED` | Run setup (ask Mangaba to set up Google Workspace) |
