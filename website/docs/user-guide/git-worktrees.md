...
title: "Git Worktrees"
description: "Run multiple Mangaba agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Mangaba Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Mangaba so each session has a clean, isolated working directory.

## Why Use Worktrees with Mangaba?

Mangaba treats the **current working directory** as the project root:

- CLI: the directory where you run `mangaba` or `mangaba chat`
...
git worktree add ../repo-feature feature/mangaba-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/mangaba-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Mangaba there:

```bash
cd ../repo-feature

# Start Mangaba in the worktree
mangaba
```
...
git worktree add ../repo-experiment-a feature/mangaba-a
git worktree add ../repo-experiment-b feature/mangaba-b
```
...
Each Mangaba process:

- Works on its own branch (`feature/mangaba-a` vs `feature/mangaba-b`).
...
- Mangaba checkpoint data under `~/.mangaba/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Mangaba experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/mangaba-checkpoints-docs`, `feature/mangaba-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Mangaba from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `mangaba -w` (Automatic Worktree Mode)

Mangaba has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `mangaba/mangaba-<hash>`).
...
```bash
mangaba -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `mangaba -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Mangaba session its own clean checkout.
