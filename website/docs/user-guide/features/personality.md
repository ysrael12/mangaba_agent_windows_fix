...
description: "Customize Mangaba Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Mangaba Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `MANGABA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Mangaba is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Mangaba now seeds a default `SOUL.md` automatically in:

```text
~/.mangaba/SOUL.md
```

More precisely, it uses the current instance's `MANGABA_HOME`, so if you run Mangaba with a custom home directory, it will use:

```text
$MANGABA_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Mangaba creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Mangaba loads `SOUL.md` only from `MANGABA_HOME`
- Mangaba does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Mangaba falls back to a built-in default identity
...
If Mangaba loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `MANGABA_HOME`, the personality belongs to the Mangaba instance itself.

That also makes it easier to teach users:
- "Edit `~/.mangaba/SOUL.md` to change Mangaba' default personality."
...
- how Mangaba should handle uncertainty, disagreement, or ambiguity
...
## What Mangaba injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Mangaba falls back to a built-in default identity ("You are Mangaba Agent, an intelligent AI assistant created by Dheiver Santos..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Mangaba ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Mangaba, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Mangaba its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.mangaba/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.mangaba/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-mangaba)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Mangaba speaks
- `display.skin` and `/skin` affect how Mangaba looks in the terminal
