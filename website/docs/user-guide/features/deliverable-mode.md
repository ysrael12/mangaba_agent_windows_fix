...
When Mangaba Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.mangaba/config.yaml` under `agent.custom_instructions`.
...
If you use Mangaba' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.mangaba/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Mangaba Agent's deliverable
