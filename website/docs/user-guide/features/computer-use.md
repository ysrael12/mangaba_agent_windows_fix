...
Mangaba Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
mangaba computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `mangaba computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `mangaba tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or Mangaba app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   mangaba -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.mangaba/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Mangaba refreshes the binary in two
places so you don't get stuck on a stale release:

- **`mangaba update`** — when you update Mangaba itself, if `cua-driver` is
...
- **`mangaba computer-use install --upgrade`** — manual force-refresh.
...
`mangaba computer-use status` shows the installed version next to the
binary path.
...
Mangaba applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.mangaba/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Mangaba applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `MANGABA_CUA_DRIVER_VERSION`
...
```
MANGABA_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
MANGABA_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
MANGABA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`mangaba computer-use install` to fetch the cua-driver binary, or run
`mangaba tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/dheiver2/mangaba-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
