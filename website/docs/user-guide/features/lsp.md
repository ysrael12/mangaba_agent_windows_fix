...
Mangaba runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Mangaba
...
1. Mangaba captures a baseline of current diagnostics for the file.
...
…). Mangaba auto-detects the binary on PATH or in
`<MANGABA_HOME>/lsp/bin/`.
...
`node_modules` tree — Mangaba installs both packages together when you
run `mangaba lsp install typescript` or auto-install fires on first
use.

## CLI

```
mangaba lsp status          # service state + per-server install status
mangaba lsp list            # registry, optionally --installed-only
mangaba lsp install <id>    # eagerly install one server
mangaba lsp install-all     # try every server with a known recipe
mangaba lsp restart         # tear down running clients
mangaba lsp which <id>      # print resolved binary path
```

`mangaba lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <MANGABA_HOME>/lsp/bin
...
When `install_strategy: auto`, Mangaba installs binaries into
`<MANGABA_HOME>/lsp/bin/`. NPM packages land in
`<MANGABA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Mangaba-owned and is
...
Servers are kept alive for the life of the Mangaba process. There's
...
**`mangaba lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<MANGABA_HOME>/lsp/bin/`. Run
`mangaba lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `mangaba lsp status`**
...
When `mangaba lsp status` shows a `Backend warnings` section, install
...
`~/.mangaba/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.mangaba/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `mangaba lsp restart` to clear the set;
