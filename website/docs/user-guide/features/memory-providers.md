...
Mangaba Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
mangaba memory setup      # interactive picker + configuration
mangaba memory status     # check what's active
mangaba memory off        # disable external provider
```

You can also select the active memory provider via `mangaba plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.mangaba/config.yaml`:
...
When a memory provider is active, Mangaba automatically:
...
**Setup Wizard:**
```bash
mangaba memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `mangaba honcho setup` command still works (it now redirects to `mangaba memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$MANGABA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$MANGABA_HOME/honcho.json` > `~/.mangaba/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/mangaba-ai/mangaba-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/mangaba).
...
:::tip Migrating from `mangaba honcho`
If you previously used `mangaba honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Mangaba profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Mangaba profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Mangaba profile. Host key `mangaba` → default; `mangaba.<profile>` for others. |
...
```bash
mangaba profile create coder --clone
```

`--clone` creates a `mangaba.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Mangaba profile, creates host blocks for any profile without one, inherits settings from the default `mangaba` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/mangaba-ai/mangaba-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/mangaba).
...
mangaba memory setup    # select "openviking"
# Or manually:
mangaba config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.mangaba/.env
```
...
**Setup:**
```bash
mangaba memory setup    # select "mem0"
# Or manually:
mangaba config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.mangaba/.env
```

**Config:** `$MANGABA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `mangaba-user` | User identifier |
| `agent_id` | `mangaba` | Agent identifier |
...
**Setup:**
```bash
mangaba memory setup    # select "hindsight"
# Or manually:
mangaba config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.mangaba/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p mangaba ui start`

**Config:** `$MANGABA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `mangaba` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Mangaba Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/dheiver2/mangaba-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
mangaba memory setup    # select "holographic"
# Or manually:
mangaba config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.mangaba-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MANGABA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
mangaba memory setup    # select "retaindb"
# Or manually:
mangaba config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.mangaba/.env
```
...
- Knowledge tree stored at `$MANGABA_HOME/byterover/` (profile-scoped)
...
**Config:** `$MANGABA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `mangaba` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `mangaba-{identity}` → `mangaba-coder`) to isolate memories per Mangaba profile
...
- **Local storage providers** (Holographic, ByteRover) use `$MANGABA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$MANGABA_HOME/` so each profile has its own credentials
