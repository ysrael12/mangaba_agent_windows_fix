...
chat completions — use your Mangaba-managed provider subscription as their
...
| Use case | "Use Mangaba as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Mangaba stores
the refresh token in `~/.mangaba/auth.json` — the same place all Mangaba
...
```
Starting Mangaba proxy for Nous Portal
...
Model:      Mangaba-4-70B    # or Mangaba-4.3-36B, Mangaba-4-405B
```
...
```
Mangaba proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `mangaba login nous`. If you see
...
supports; check `portal.dheiver2.com/models`.
...
INFERENCE_TEXT_MODEL=Mangaba-4-70B
```
...
```bash
mangaba proxy start --host 0.0.0.0 --port 8645
```
...
[portal.dheiver2.com](https://portal.dheiver2.com).
...
`mangaba_cli/proxy/adapters/<provider>.py` and registering it in
