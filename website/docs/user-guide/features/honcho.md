...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Mangaba's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Mangaba instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
mangaba memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.mangaba/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$MANGABA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `mangaba` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Mangaba syncs them back at session init.
...
The `mangaba honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `mangaba memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
mangaba honcho status          # Connection status, config, and key settings
mangaba honcho setup           # Redirects to `mangaba memory setup`
mangaba honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
mangaba honcho peer            # Show or update peer names + dialectic reasoning level
mangaba honcho mode            # Show or set recall mode (hybrid/context/tools)
mangaba honcho tokens          # Show or set token budget for context and dialectic
mangaba honcho identity        # Seed or show the AI peer's Honcho identity
mangaba honcho sync            # Sync Honcho config to all existing profiles
mangaba honcho peers           # Show peer identities across all profiles
mangaba honcho sessions        # List known Honcho session mappings
mangaba honcho map             # Map current directory to a Honcho session name
mangaba honcho enable          # Enable Honcho for the active profile
mangaba honcho disable         # Disable Honcho for the active profile
mangaba honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `mangaba honcho`

If you previously used the standalone `mangaba honcho setup`:
...
No re-login or re-setup needed. Run `mangaba memory setup` and select "honcho" — the wizard detects your existing config.
