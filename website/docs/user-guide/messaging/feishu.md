...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Mangaba Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Mangaba Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Mangaba opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Mangaba responds to every message. |
| Group chats | Mangaba responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Mangaba will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `mangaba gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Mangaba runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Mangaba behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Mangaba starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Mangaba responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Mangaba responds to @mentions from any user in any group. |
| `allowlist` | Mangaba only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Mangaba ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Mangaba read all group traffic without requiring an @mention:
...
Mangaba auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Mangaba ignores messages sent by other bots. Enable bot-to-bot messaging when you want Mangaba to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Mangaba. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `mangaba update --gateway` needs confirmation, the adapter records the selected answer in Mangaba's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Mangaba reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.mangaba/feishu_comment_rules.json` (pairing grants in `~/.mangaba/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `MANGABA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `MANGABA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `MANGABA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `MANGABA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.mangaba/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `MANGABA_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `MANGABA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `MANGABA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `MANGABA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `MANGABA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `MANGABA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `mangaba gateway setup` |
| `Another local Mangaba gateway is already using this Feishu app_id` | Only one Mangaba instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Mangaba can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `mangaba-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
