...
Connect Mangaba to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Mangaba supports two WeCom integration modes:
...
3. Mangaba decrypts the message, queues it for the agent
...
(Use `mangaba gateway start` only after `mangaba gateway install` has registered the systemd/launchd service.)
