...
description: "Chat with Mangaba from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Mangaba from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Mangaba](/docs/guides/use-voice-mode-with-mangaba).
...
```mermaid
flowchart TB
    subgraph Gateway["Mangaba Gateway"]
...
```bash
mangaba gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
mangaba gateway              # Run in foreground
mangaba gateway setup        # Configure messaging platforms interactively
mangaba gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo mangaba gateway install --system   # Linux only: install a boot-time system service
mangaba gateway start        # Start the default service
mangaba gateway stop         # Stop the default service
mangaba gateway status       # Check default service status
mangaba gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Mangaba Agent to the latest version |
...
Configure per-platform overrides in `~/.mangaba/gateway.json`:
...
mangaba pairing approve telegram XKGH5N7P

# Other pairing commands:
mangaba pairing list          # View pending + approved users
mangaba pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Mangaba appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.mangaba/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.mangaba/config.yaml`:
...
```bash
MANGABA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
mangaba gateway install               # Install as user service
mangaba gateway start                 # Start the service
mangaba gateway stop                  # Stop the service
mangaba gateway status                # Check status
journalctl --user -u mangaba-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo mangaba gateway install --system
sudo mangaba gateway start --system
sudo mangaba gateway status --system
journalctl -u mangaba-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Mangaba will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Mangaba installations on the same machine (with different `MANGABA_HOME` directories), each gets its own systemd service name. The default `~/.mangaba` uses `mangaba-gateway`; other installations use `mangaba-gateway-<hash>`. The `mangaba gateway` commands automatically target the correct service for your current `MANGABA_HOME`.
:::

### macOS (launchd)

```bash
mangaba gateway install               # Install as launchd agent
mangaba gateway start                 # Start the service
mangaba gateway stop                  # Stop the service
mangaba gateway status                # Check status
tail -f ~/.mangaba/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.mangaba.gateway.plist`. It includes three environment variables:
...
- **MANGABA_HOME** — scopes the gateway to your Mangaba installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `mangaba gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `MANGABA_HOME` directory gets its own launchd label. The default `~/.mangaba` uses `ai.mangaba.gateway`; other installations use `ai.mangaba.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `mangaba-cli` | Full access |
| Telegram | `mangaba-telegram` | Full tools including terminal |
| Discord | `mangaba-discord` | Full tools including terminal |
| WhatsApp | `mangaba-whatsapp` | Full tools including terminal |
| Slack | `mangaba-slack` | Full tools including terminal |
| Google Chat | `mangaba-google_chat` | Full tools including terminal |
| Signal | `mangaba-signal` | Full tools including terminal |
| SMS | `mangaba-sms` | Full tools including terminal |
| Email | `mangaba-email` | Full tools including terminal |
| Home Assistant | `mangaba-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `mangaba-mattermost` | Full tools including terminal |
| Matrix | `mangaba-matrix` | Full tools including terminal |
| DingTalk | `mangaba-dingtalk` | Full tools including terminal |
| Feishu/Lark | `mangaba-feishu` | Full tools including terminal |
| WeCom | `mangaba-wecom` | Full tools including terminal |
| WeCom Callback | `mangaba-wecom-callback` | Full tools including terminal |
| Weixin | `mangaba-weixin` | Full tools including terminal |
| BlueBubbles | `mangaba-bluebubbles` | Full tools including terminal |
| QQBot | `mangaba-qqbot` | Full tools including terminal |
| Yuanbao | `mangaba-yuanbao` | Full tools including terminal |
| Microsoft Teams | `mangaba-teams` | Full tools including terminal |
| API Server | `mangaba-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `mangaba-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.mangaba/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
