...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Mangaba Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Mangaba to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.mangaba/.env`:
...
All standard Mangaba commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.mangaba/logs/gateway.log`
...
4. Consider enabling verbose logging: `MANGABA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Mangaba automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
MANGABA_LOG_LEVEL=debug mangaba gateway
```
...
```bash
mangaba chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
