...
This page covers setting up inference providers for Mangaba Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Mangaba.

## Inference Providers

You need at least one way to connect to an LLM. Use `mangaba model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `mangaba model` (OAuth, subscription-based) |
| **OpenAI Codex** | `mangaba model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `mangaba model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `mangaba model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `mangaba model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.mangaba/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.mangaba/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.mangaba/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.mangaba/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.mangaba/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.mangaba/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.mangaba/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.mangaba/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.mangaba/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.mangaba/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.mangaba/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `mangaba model` → "xAI Grok OAuth (SuperGrok Subscription)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.mangaba/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.mangaba/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.mangaba/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.mangaba/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.mangaba/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.mangaba/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.mangaba/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.mangaba/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.mangaba/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `mangaba model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `mangaba model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `mangaba model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Mangaba auto-provisions your free tier on first request
```

Mangaba ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.mangaba/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `MANGABA_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Mangaba works unchanged
...
| Workspace / Standard / Enterprise account | Set `MANGABA_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Mangaba detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Mangaba shows
...
```bash
MANGABA_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
MANGABA_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Mangaba stores the resulting credentials in its own auth store under `~/.mangaba/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Mangaba marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `mangaba auth add codex-oauth` (or `mangaba model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Mangaba routes these tasks to your **main chat model** — the same model you picked in `mangaba model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/docs/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `mangaba setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `mangaba model` or per-tool from `mangaba tools`. Inspect routing at any time with `mangaba portal status`.
:::

### Two Commands for Model Management

Mangaba has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`mangaba model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Mangaba chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `mangaba model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `mangaba model`, complete the provider setup, then start a new session.

### Nous Portal

Subscription-based access to Mangaba-4 models (`Mangaba-4-70B`, `Mangaba-4.3-36B`, `Mangaba-4-405B`) via Dheiver Santos's portal. Two paths:

- **Fresh install:** `mangaba setup --portal` — runs OAuth, sets Nous as your provider, and offers the [Tool Gateway](/docs/user-guide/features/tool-gateway) in one command.
- **Existing install:** `mangaba model`, pick **Nous Portal**, sign in through the browser.

Either path stores a long-lived refresh token at `~/.mangaba/auth.json`. Check status with `mangaba portal status`.
...
Mangaba mints a short-lived JWT from your stored Nous refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is quarantined locally so Mangaba stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `mangaba auth add nous` to log in again; the quarantine clears on the next successful login.
...
When you authenticate via `mangaba model` → Anthropic OAuth (or via `mangaba auth add anthropic --type oauth`), Mangaba routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Mangaba — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
mangaba chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `mangaba model`
# Mangaba will use Claude Code's credential store directly when available
mangaba model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
mangaba chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
mangaba chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `mangaba model`, Mangaba prefers Claude Code's own credential store over copying the token into `~/.mangaba/.env`. That keeps refreshable Claude credentials refreshable.
...
Mangaba supports GitHub Copilot as a first-class provider with two modes:
...
```bash
mangaba chat --provider copilot --model gpt-5.4
```
...
If no token is found, `mangaba model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `mangaba model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `mangaba model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Mangaba
Mangaba sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Mangaba now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Mangaba therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
mangaba chat --provider copilot-acp --model copilot-acp
...
| `MANGABA_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `MANGABA_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
mangaba chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.mangaba/.env

# z.ai / ZhipuAI GLM
mangaba chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.mangaba/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
mangaba chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.mangaba/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
mangaba chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.mangaba/.env

# MiniMax (global endpoint)
mangaba chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.mangaba/.env

# MiniMax (China endpoint)
mangaba chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.mangaba/.env

# Qwen Cloud / DashScope (Qwen models)
mangaba chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.mangaba/.env

# Xiaomi MiMo
mangaba chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.mangaba/.env

# Tencent TokenHub (Hy3 Preview)
mangaba chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.mangaba/.env

# Arcee AI (Trinity models)
mangaba chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.mangaba/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
mangaba chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.mangaba/.env
```
...
When using the Z.AI / GLM provider, Mangaba automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.mangaba/.env` and pick xAI in `mangaba model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok Subscription)** in `mangaba model`, or run `mangaba auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Mangaba runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Mangaba automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `mangaba tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.
...
# Short alias
mangaba chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `mangaba model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Mangaba auto-discovers the available models.
...
mangaba chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 mangaba chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `mangaba model`, sign in through the browser, and Mangaba persists the refresh token.

```bash
mangaba model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.mangaba/auth.json

mangaba chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `MANGABA_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Mangaba exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
mangaba chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `mangaba model`, sign in through the browser, and Mangaba persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
mangaba chat   # uses api.minimax.io/anthropic endpoint
```
...
mangaba chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.mangaba/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 mangaba chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Mangaba automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
mangaba chat --provider gmi --model deepseek-ai/DeepSeek-R1
# Requires: GMI_API_KEY in ~/.mangaba/.env
```
...
```bash
# StepFun
mangaba chat --provider stepfun --model step-3-mini
# Requires: STEPFUN_API_KEY in ~/.mangaba/.env
```
...
```bash
# Use any available model
mangaba chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.mangaba/.env

# Short alias
mangaba chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Mangaba Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Mangaba at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Mangaba — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `mangaba setup` or config migration. Use `mangaba model` or edit `config.yaml` directly.
:::
...
:::warning mangaba model vs /model
**`mangaba model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Mangaba chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `mangaba model`, set up the new provider, then start a new session.
:::
...
When switching providers, Mangaba persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Mangaba.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Mangaba) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `mangaba` (Qwen 2.5, Mangaba 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Mangaba to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Mangaba won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Mangaba 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Mangaba will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Mangaba) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Mangaba Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Mangaba config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Mangaba config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Mangaba.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Mangaba inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser mangaba` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Mangaba's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Mangaba thinks the context is
...
Mangaba auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Mangaba uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Mangaba with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Mangaba.
...
Then configure Mangaba with `mangaba model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Mangaba with `mangaba model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `mangaba model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Mangaba uses this to decide when to compress history and to validate API requests.
...
Mangaba uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`mangaba model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Mangaba uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `mangaba model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Mangaba will merge it into each chat-completions request for that endpoint:
...
The `mangaba model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `mangaba model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.mangaba/config.yaml` and the matching API key goes in `~/.mangaba/.env`.
...
Together's `/v1/models` endpoint works, so `mangaba model` can auto-discover available models.
...
:::tip Troubleshooting
- `mangaba doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `mangaba model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `mangaba model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Mangaba at your instance (no API key needed):
   ```bash
   mangaba config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.mangaba/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.mangaba/config.yaml`:
...
Configure a chain of backup providers Mangaba tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `mangaba fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/user-guide/features/fallback-providers).
:::
