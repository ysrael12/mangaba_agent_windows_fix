---
sidebar_position: 8
title: "Use Voice Mode with Mangaba"
description: "A practical guide to setting up and using Mangaba voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Mangaba
...
- you want Mangaba sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Mangaba.
...
## Step 1: make sure normal Mangaba works first
...
Mangaba supports both local and cloud speech stacks.
...
### If you use `mangaba setup`

If you choose NeuTTS in the setup wizard, Mangaba checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Mangaba transcribes and responds
...
- asking Mangaba to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Mangaba loop.

## Tuning CLI behavior

### Silence threshold

If Mangaba starts/stops too aggressively, tune:
...
Mangaba stays a normal chat bot, but can speak replies.
...
- you want Mangaba to function like a portable research or ops assistant
...
Mangaba joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Mangaba detects speech boundaries
- transcripts are posted in the associated text channel
- Mangaba responds in text and audio
...
1. get text Mangaba working
2. install `mangaba-agent[voice]`
