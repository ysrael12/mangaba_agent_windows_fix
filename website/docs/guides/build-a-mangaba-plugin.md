...
title: "Build a Mangaba Plugin"
description: "Step-by-step guide to building a complete Mangaba plugin with tools, hooks, data files, and skills"
---

# Build a Mangaba Plugin

This guide walks through building a complete Mangaba plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Mangaba has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/docs/user-guide/features/tts#voice-message-transcription-stt) — set `MANGABA_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.mangaba/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/docs/user-guide/features/skills) — `mangaba skills tap add <repo>` · [Publishing a tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.mangaba/plugins/calculator
cd ~/.mangaba/plugins/calculator
```
...
This tells Mangaba: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Mangaba may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `mangaba my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Mangaba continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `MANGABA_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
MANGABA_PLUGINS_DEBUG=1 mangaba plugins list
```
...
The same logs are always written to `~/.mangaba/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
mangaba logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `mangaba plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.mangaba/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.mangaba/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.mangaba/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.mangaba/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `mangaba plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Mangaba into installing arbitrary packages — only specs Mangaba itself ships are eligible |
...
auditable in `~/.mangaba/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Mangaba injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: mangaba-agent"}
...
- **The system prompt is Mangaba's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `mangaba <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for mangaba my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: mangaba my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for mangaba my-plugin."""
...
After registration, users can run `mangaba my-plugin status`, `mangaba my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `mangaba name` in a terminal |
...
Mangaba has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.mangaba/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `mangaba gateway status` without SDK instantiation.
...
Mangaba also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Mangaba without any Python plugin. Declare them in `~/.mangaba/config.yaml`:
...
Mangaba connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/docs/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.mangaba/hooks/<name>/`:

```yaml
# ~/.mangaba/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.mangaba/hooks/long-task-alert/handler.py
...
mangaba skills search my-workflow --source myorg/skills-repo
mangaba skills install myorg/skills-repo/my-workflow
```
...
For STT, point `MANGABA_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."mangaba_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install mangaba-plugin-calculator
# Plugin auto-discovered on next mangaba startup
```
...
```python
# Wrong — will break if Mangaba passes extra context
def handler(args):
    ...
