...
```bash
mangaba teams-pipeline validate
```
...
```bash
mangaba teams-pipeline token-health
mangaba teams-pipeline token-health --force-refresh
```
...
```bash
mangaba teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
mangaba teams-pipeline maintain-subscriptions
mangaba teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Mangaba cron (recommended if you already run the Mangaba gateway)

Mangaba ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.mangaba/scripts/`. First create the script:

```bash
mkdir -p ~/.mangaba/scripts
cat > ~/.mangaba/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec mangaba teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.mangaba/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
mangaba cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/mangaba-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Mangaba Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=mangaba
EnvironmentFile=/etc/mangaba/env
ExecStart=/usr/local/bin/mangaba teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/mangaba-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Mangaba Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now mangaba-teams-pipeline-maintain.timer
systemctl list-timers mangaba-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/mangaba teams-pipeline maintain-subscriptions >> /var/log/mangaba/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.mangaba/.env` at the top of a wrapper script that crontab calls.
...
```bash
mangaba teams-pipeline subscriptions   # should show expirationDateTime advanced
mangaba teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
mangaba teams-pipeline list
mangaba teams-pipeline list --status failed
mangaba teams-pipeline show <job-id>
```

### Replay a stored job

```bash
mangaba teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
mangaba teams-pipeline fetch --meeting-id <meeting-id>
mangaba teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `mangaba teams-pipeline maintain-subscriptions --dry-run`
- inspect `mangaba teams-pipeline list --status failed`
...
- run `mangaba teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `mangaba teams-pipeline run`
...
- [ ] `mangaba teams-pipeline validate` returns an OK snapshot
- [ ] `mangaba teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Mangaba cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
