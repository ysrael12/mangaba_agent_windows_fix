...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `mangaba send`."
---

# Pipe Script Output to Messaging Platforms

`mangaba send` is a small, scriptable CLI that pushes a message to any
messaging platform Mangaba is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | mangaba send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `mangaba
...
mangaba send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | mangaba send --to telegram:-1001234567890

# Send a file
mangaba send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
mangaba send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
mangaba send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
mangaba send --list

# Filter by platform
mangaba send --list telegram
```
...
Any platform Mangaba ships adapters for works as a target:
...
`mangaba send` resolves the message body in this order:

1. **Positional argument** — `mangaba send --to telegram "hi"`
2. **`--file PATH`** — `mangaba send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | mangaba send --to telegram`

When stdin is a TTY (no pipe), Mangaba does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  mangaba send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `mangaba send` reuses your Mangaba config, the same script works on
any host where Mangaba is installed — no need to export bot tokens into
...
`mangaba send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  mangaba send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | mangaba send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.mangaba/bin/mangaba send \
...
```bash
./train.py --epochs 200 && \
  mangaba send --to telegram "training done" || \
  mangaba send --to telegram "training failed (exit $?)"
```
...
mangaba send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(mangaba send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `mangaba send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `mangaba send` calls
the platform's REST endpoint directly using credentials from
`~/.mangaba/.env` and `~/.mangaba/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `mangaba gateway start` and retry.
...
# Machine-readable
mangaba send --list --json
```

The listing is built from `~/.mangaba/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`mangaba gateway
...
| Approach | Multi-platform | Reuses Mangaba creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `mangaba send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`mangaba send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `mangaba send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `mangaba send` shares with cron delivery.
