...
This tutorial walks you through setting up a Telegram bot powered by Mangaba Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Mangaba Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.mangaba/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Mangaba itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Mangaba Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_mangaba_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Mangaba Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.mangaba/.env`:
...
```
[Gateway] Starting Mangaba Gateway...
...
# View live logs
journalctl --user -u mangaba-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `mangaba gateway install` to update the plist.
:::
...
```bash
mangaba gateway stop && mangaba gateway start
```
...
mangaba pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.mangaba/.env`:
...
Control how much detail the bot shows when using tools. In `~/.mangaba/config.yaml`:
...
Customize how the bot communicates by editing `~/.mangaba/SOUL.md`:

For a full guide, see [Use SOUL.md with Mangaba](/docs/guides/use-soul-with-mangaba).
...
```bash
# From the CLI
mangaba cron list          # View all scheduled jobs
mangaba cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u mangaba-gateway` (Linux) or `~/.mangaba/logs/gateway.log` (macOS) |
| Cron job output | `~/.mangaba/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.mangaba/cron/jobs.json` |
| Pairing data | `~/.mangaba/pairing/` |
| Session history | `~/.mangaba/sessions/` |
