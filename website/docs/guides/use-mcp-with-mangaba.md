---
sidebar_position: 6
title: "Use MCP with Mangaba"
description: "A practical guide to connecting MCP servers to Mangaba Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Mangaba

This guide shows how to actually use MCP with Mangaba Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Mangaba tool
- you want Mangaba to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Mangaba to internal APIs, databases, or company systems without modifying Mangaba core

Do not use MCP when:
- a built-in Mangaba tool already solves the job well
...
- Mangaba remains the agent
- MCP servers contribute tools
- Mangaba discovers those tools at startup or reload time
...
If you installed Mangaba with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Mangaba banner/status should show MCP integration when configured
- ask Mangaba what tools it has available
...
## WSL2: bridge Mangaba in WSL to Windows Chrome

This is the practical setup when:

- Mangaba runs inside WSL2
...
In this setup, Mangaba does **not** connect to Chrome directly. Instead:

- Mangaba runs in WSL
- Mangaba starts a local stdio MCP server
...
```text
Mangaba (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Mangaba stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Mangaba core browser transport
...
```bash
mangaba mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
mangaba mcp test chrome-devtools-win
```

Then start a fresh Mangaba session or run:
...
Once loaded, Mangaba can use the MCP-prefixed browser tools directly. For example:
...
If Mangaba runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Mangaba from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Mangaba from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Mangaba:
...
2. Mangaba-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Mangaba will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Mangaba to reason over a bounded workspace.
...
Now Mangaba can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Mangaba core.
...
Because Mangaba now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
