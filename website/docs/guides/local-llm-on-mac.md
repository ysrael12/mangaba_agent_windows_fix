...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Mangaba works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
Mangaba automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
MANGABA_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `MANGABA_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `MANGABA_STREAM_STALE_TIMEOUT` |
