...
Copy-paste recipes for common automation patterns. Each template uses Mangaba's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`mangaba webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `mangaba webhook subscribe`) |
...
```bash
mangaba cron create "0 2 * * *" \
  "You are a project manager triaging the dheiver2/mangaba-agent GitHub repo.

1. Run: gh issue list --repo dheiver2/mangaba-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
mangaba webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
mangaba cron create "0 9 * * 1" \
  "Scan the dheiver2/mangaba-agent repo for documentation drift.

1. Run: gh pr list --repo dheiver2/mangaba-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (mangaba_cli/commands.py, mangaba_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (mangaba_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
mangaba cron create "0 6 * * *" \
  "Run a dependency security audit on the mangaba-agent project.

1. cd ~/.mangaba/mangaba-agent && source .venv/bin/activate
...
```bash
mangaba webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
mangaba webhook subscribe alert-triage \
...
```python title="~/.mangaba/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Mangaba-Monitor/1.0"})
...
mangaba cron create "every 30m" \
  ...
  --script ~/.mangaba/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
mangaba cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Mangaba Agent development." \
...
```bash
mangaba webhook subscribe github-issues \
  --events "issues" \
...
```bash
mangaba webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
mangaba webhook subscribe stripe-payments \
...
```bash
mangaba cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the mangaba-agent codebase.
