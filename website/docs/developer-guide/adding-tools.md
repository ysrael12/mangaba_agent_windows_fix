...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Mangaba Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Mangaba tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Mangaba core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Mangaba Plugin](/docs/guides/build-a-mangaba-plugin)
...
2. **`toolsets.py`** — add tool name to `_MANGABA_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `mangaba_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `mangaba_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `mangaba chat -q "Use the weather tool for London"`
