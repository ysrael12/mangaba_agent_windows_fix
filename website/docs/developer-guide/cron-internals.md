...
description: "How Mangaba stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `mangaba_cli/cron.py` | CLI `mangaba cron` subcommands |
...
Jobs are stored in `~/.mangaba/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `mangaba cron` commands are run or during active CLI sessions.
...
```python
# ~/.mangaba/scripts/check_competitors.py
...
2. **Environment variable** — `MANGABA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.mangaba/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `mangaba cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `mangaba cron` CLI provides direct job management:

```bash
mangaba cron list                    # Show all jobs
mangaba cron create                  # Interactive job creation (alias: add)
mangaba cron edit <job_id>           # Edit job configuration
mangaba cron pause <job_id>          # Pause a running job
mangaba cron resume <job_id>         # Resume a paused job
mangaba cron run <job_id>            # Trigger immediate execution
mangaba cron remove <job_id>         # Delete a job
```
