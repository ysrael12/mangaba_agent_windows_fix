...
Mangaba Agent uses a SQLite database (`~/.mangaba/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `mangaba_state.py`


## Architecture Overview

```
~/.mangaba/state.db (SQLite, WAL mode)
...
Multiple mangaba processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.mangaba/state.db
...
Default path: `~/.mangaba/state.db`

This is derived from `mangaba_constants.get_mangaba_home()` which resolves to
`~/.mangaba/` by default, or the value of `MANGABA_HOME` environment variable.
