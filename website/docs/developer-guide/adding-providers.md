...
description: "How to add a new inference provider to Mangaba Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Mangaba can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `mangaba model` menu entries
...
1. `mangaba_cli/auth.py` decides how credentials are found.
2. `mangaba_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `mangaba_cli/models.py` and `mangaba_cli/main.py` make the provider show up in the CLI. (`mangaba_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `mangaba_cli/auth.py`
2. `mangaba_cli/models.py`
3. `mangaba_cli/runtime_provider.py`
4. `mangaba_cli/main.py`
...
:::tip
`mangaba_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `mangaba setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$MANGABA_HOME/plugins/model-providers/` both get picked up.
...
7. `mangaba model` menu includes the provider
8. `mangaba setup` wizard delegates to `main.py` automatically
9. `provider:model` alias syntax works
10. Runtime resolver returns the correct `base_url` and `api_key`
11. `MANGABA_INFERENCE_PROVIDER` env-var override accepts the provider id
12. Fallback model activation can switch into the provider cleanly

User plugins at `$MANGABA_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `mangaba model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `mangaba_cli/auth.py`
- `_PROVIDER_LABELS` in `mangaba_cli/models.py`
- `_PROVIDER_ALIASES` in both `mangaba_cli/auth.py` and `mangaba_cli/models.py`
- CLI `--provider` choices in `mangaba_cli/main.py`
...
## Step 2: Add auth metadata in `mangaba_cli/auth.py`
...
- What env vars should Mangaba check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `mangaba_cli/models.py`
...
## Step 4: Resolve runtime data in `mangaba_cli/runtime_provider.py`
...
Be careful with API-key precedence. Mangaba already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `mangaba_cli/main.py`

A provider is not discoverable until it shows up in the interactive `mangaba model` flow.

Update these in `mangaba_cli/main.py`:
...
:::tip
`mangaba_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `mangaba model` and `mangaba setup` automatically.
:::
...
When you add a native provider, double-check that Mangaba is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m mangaba_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m mangaba_cli.main model
...
- [ ] `ProviderConfig` added in `mangaba_cli/auth.py`
- [ ] aliases added in `mangaba_cli/auth.py` and `mangaba_cli/models.py`
- [ ] model catalog added in `mangaba_cli/models.py`
- [ ] runtime branch added in `mangaba_cli/runtime_provider.py`
- [ ] CLI wiring added in `mangaba_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `mangaba model` but not `mangaba setup`
