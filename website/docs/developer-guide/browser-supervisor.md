...
[PR #12550](https://github.com/dheiver2/mangaba-agent/pull/12550) proposed a
...
(`mangaba-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Mangaba `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `mangaba-acp`, `mangaba-api-server`, core toolsets (gated on CDP reachability)
...
- `mangaba_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
