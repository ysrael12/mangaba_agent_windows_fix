...
The ACP adapter wraps Mangaba' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
mangaba acp / mangaba-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.mangaba/.env
  -> configure stderr logging
  -> construct MangabaACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'mangaba-agent[acp]==<version>' mangaba-acp`, pointed at the `mangaba-agent` PyPI release.
...
- `allow_once` -> Mangaba `once`
- `allow_always` -> Mangaba `always`
- reject options -> Mangaba `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Mangaba tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["mangaba-acp"])
...
Instead it reuses Mangaba' runtime resolver:

- `acp_adapter/auth.py`
- `mangaba_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Mangaba provider/credentials. It also always advertises a terminal setup auth method (`mangaba-setup`, args `--setup`) so first-run registry clients can open Mangaba' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.mangaba/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `mangaba-acp` toolset definition
- `mangaba_cli/main.py` — `mangaba acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `mangaba-acp` script
