...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Mangaba Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Mangaba Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Mangaba core? Start with [Build a Mangaba Plugin](../guides/build-a-mangaba-plugin.md)
- Building a new built-in core tool for Mangaba itself? Start with [Adding Tools](./adding-tools.md)
...
```bash
git clone --recurse-submodules https://github.com/dheiver2/mangaba-agent.git
cd mangaba-agent
...
```bash
mkdir -p ~/.mangaba/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.mangaba/config.yaml
touch ~/.mangaba/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.mangaba/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/mangaba" ~/.local/bin/mangaba
...
- **Profile-safe paths**: Never hardcode `~/.mangaba` — use `get_mangaba_home()` from `mangaba_constants` for code paths and `display_mangaba_home()` for user-facing messages. See [AGENTS.md](https://github.com/dheiver2/mangaba-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Mangaba officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
Mangaba has terminal access. Security matters.
...
2. **Test manually**: Run `mangaba` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/dheiver2/mangaba-agent/issues)
- Include: OS, Python version, Mangaba version (`mangaba version`), full error traceback
...
- **Discord**: [discord.gg/dheiver2](https://discord.gg/dheiver2)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/dheiver2/mangaba-agent/blob/main/LICENSE).
