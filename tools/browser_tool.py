...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    mangaba_node_bin = str(mangaba_home / "node" / "bin")
    mangaba_node_root = str(mangaba_home / "node")
    mangaba_nm_bin = str(mangaba_home / "node_modules" / ".bin")
    return [mangaba_node_bin, mangaba_node_root, mangaba_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.mangaba/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.mangaba/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Mangaba silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Mangaba CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-mangaba_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed mangaba processes — this way every clean mangaba exit sweeps
    ...
    # Sweep orphans from other crashed mangaba processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live mangaba processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current mangaba PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live mangaba process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning mangaba process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced mangaba PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent mangaba instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-mangaba_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live mangaba process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Mangaba-managed
    ...
    # Build an extended search PATH including Mangaba-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                mangaba_nm = str(get_mangaba_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=mangaba_nm)
            if not recheck:
                mangaba_node_bin = str(get_mangaba_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=mangaba_node_bin)
            if not recheck:
                mangaba_node_root = str(get_mangaba_home() / "node")
                recheck = shutil.which("agent-browser", path=mangaba_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this mangaba PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = mangaba_home / "browser_recordings"
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/dheiver2/mangaba-agent:latest")
                else:
