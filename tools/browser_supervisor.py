...
One ``CDPSupervisor`` runs per Mangaba ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "mangaba-dialog-bridge.invalid"
...
(() => {
  if (window.__mangabaDialogBridgeInstalled) return;
  window.__mangabaDialogBridgeInstalled = true;
  const ENDPOINT = "http://mangaba-dialog-bridge.invalid/";
