...
# Must match mangaba_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (mangaba-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("mangaba-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``mangaba-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. mangaba-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.mangaba/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    import mangaba_agent.model_tools as _model_tools
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'mangaba auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (mangaba_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Mangaba subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
