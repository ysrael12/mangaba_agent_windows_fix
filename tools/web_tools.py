...
Backend is selected during ``mangaba tools`` setup (web.backend in config.yaml).
When available, Mangaba can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.mangaba/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``mangaba tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `mangaba tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "dheiver2.com" or host.endswith(".dheiver2.com")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `mangaba tools` to set one up."
                ),
            }
        else:
            logger.info(
