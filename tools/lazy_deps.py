"""
Lazy dependency installer for opt-in Mangaba Agent backends.

Many Mangaba features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``mangaba-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``mangaba tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("MANGABA_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``mangaba update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``mangaba_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``mangaba update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``mangaba update``. Never raises; lazy-install failures
