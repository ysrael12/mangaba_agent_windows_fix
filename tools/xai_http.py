...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``mangaba tools`` repaint, tool-registration scans,
    ...
    2. ``~/.mangaba/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.mangaba/.env`` first, then ``os.environ``.

    Wraps :func:`mangaba_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from mangaba_cli.config import get_env_value as _mangaba_get_env_value

        value = _mangaba_get_env_value(name)
...
def mangaba_xai_user_agent() -> str:
    """Return a stable Mangaba-specific User-Agent for xAI HTTP calls."""
    try:
        from mangaba_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Mangaba-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Mangaba-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``mangaba_cli.config.get_env_value`` so keys
    stored in ``~/.mangaba/.env`` (the standard Mangaba location) are honored —
