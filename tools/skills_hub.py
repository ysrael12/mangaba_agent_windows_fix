#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Mangaba Skills Hub.
...
Used by mangaba_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            mangaba_meta = metadata.get("mangaba", {})
            if isinstance(mangaba_meta, dict):
                tags = mangaba_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            mangaba_meta = metadata.get("mangaba", {})
            if isinstance(mangaba_meta, dict):
                raw_tags = mangaba_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Dheiver Santos) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                mangaba_meta = meta_block.get("mangaba", {})
                if isinstance(mangaba_meta, dict):
                    tags = mangaba_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Mangaba centralized index source
# ---------------------------------------------------------------------------

MANGABA_INDEX_URL = "https://dheiver2.github.io/mangaba-agent/docs/api/skills-index.json"
MANGABA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "mangaba-index.json"
MANGABA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_mangaba_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for MANGABA_INDEX_TTL seconds to avoid repeated
    ...
    try:
        resp = httpx.get(MANGABA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Mangaba index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Mangaba index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class MangabaIndexSource(SkillSource):
    """Skill source backed by the centralized Mangaba Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_mangaba_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "mangaba-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "mangaba-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        MangabaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "mangaba-index"
