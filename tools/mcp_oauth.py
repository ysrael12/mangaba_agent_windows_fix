...
This module provides the glue:
    - ``MangabaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Mangaba Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses MANGABA_HOME so each profile gets its own OAuth tokens.
...
# ---------------------------------------------------------------------------
# MangabaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MangabaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MANGABA_HOME/mcp-tokens/<server_name>.json         -- tokens
        MANGABA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        MANGABA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Mangaba records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Mangaba.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://dheiver2.github.io/mangaba-agent/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Mangaba Agent")
    scope = cfg.get("scope")
