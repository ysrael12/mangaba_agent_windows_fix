...
it is automatically downloaded from GitHub releases to $MANGABA_HOME/bin/tirith.
...
def _get_mangaba_home() -> str:
    """Return the Mangaba home directory, respecting MANGABA_HOME env var."""
    return str(get_mangaba_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_mangaba_home(), ".tirith-install-failed")
...
def _mangaba_bin_dir() -> str:
    """Return $MANGABA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_mangaba_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MANGABA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_mangaba_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MANGABA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MANGABA_HOME/bin/tirith
    ...
    mangaba_bin = os.path.join(_mangaba_bin_dir(), "tirith")
    if os.path.isfile(mangaba_bin) and os.access(mangaba_bin, os.X_OK):
        _resolved_path = mangaba_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return mangaba_bin
