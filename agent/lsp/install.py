...
appropriate.  All installs go to a Mangaba-owned bin staging dir,
``<MANGABA_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``mangaba lsp
  status``.
...
# ``<MANGABA_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def mangaba_lsp_bin_dir() -> Path:
    """Return the Mangaba-owned bin staging dir for LSP servers."""
    home = os.environ.get("MANGABA_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".mangaba")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = mangaba_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = mangaba_lsp_bin_dir().parent  # <MANGABA_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = mangaba_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a mangaba-owned target dir.
    ...
    console script.
    """
    pip_target = mangaba_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = mangaba_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``mangaba lsp status`` CLI to give users a quick
