...
Plugins built on Mangaba Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Mangaba
...
:class:`~mangaba_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Mangaba' main config
...
handles every provider, fallback chain, and per-task override Mangaba
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`mangaba_cli.plugins.PluginContext`
