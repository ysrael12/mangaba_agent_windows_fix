...
Storage
-------
Bundles live in ``~/.mangaba/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MANGABA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``mangaba bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MANGABA_HOME.

    Honors ``MANGABA_BUNDLES_DIR`` for tests; falls back to
    ``<MANGABA_HOME>/skill-bundles``.
    """
    override = os.environ.get("MANGABA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_mangaba_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `mangaba bundles` CLI subcommand.
# ---------------------------------------------------------------------------
