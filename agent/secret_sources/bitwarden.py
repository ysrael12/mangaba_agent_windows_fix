...
Mangaba pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.mangaba/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<mangaba_home>/bin/bws`` on
  first use.  Mangaba pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.mangaba/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``mangaba`` invocations don't
  hammer the API.
* Failures NEVER block Mangaba startup.  Missing binary, no network,
...
# In-process cache so repeated load_mangaba_dotenv() calls (CLI startup,
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<mangaba_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _mangaba_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``mangaba secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _mangaba_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="mangaba-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "mangaba-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from mangaba_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_mangaba_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `mangaba secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `mangaba secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `mangaba secrets bitwarden setup` to install."
        )
        return result
