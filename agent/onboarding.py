...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Mangaba starts and finds ``~/.openclaw/``.

    Points users at ``mangaba claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``mangaba claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Mangaba, run "
        "`mangaba claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `mangaba claw cleanup` (renames it to ~/.openclaw.pre-migration — "
