...
Owns one Codex thread per Mangaba session. Drives `turn/start`, consumes
...
# Mangaba' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_MANGABA_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Mangaba session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _MANGABA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("MANGABA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="mangaba",
            client_title="Mangaba Agent",
            client_version=_get_mangaba_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Mangaba' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Mangaba' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own mangaba-tools callback we
            # auto-accept — the user already approved Mangaba' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Mangaba approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Mangaba returns 'once', 'session', 'always', or 'deny'.
...
def _get_mangaba_version() -> str:
    """Best-effort Mangaba version string for codex's userAgent line."""
    try:
