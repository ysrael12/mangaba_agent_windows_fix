...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.mangaba/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from mangaba_cli.config import load_config as _load_pc_cfg
    ...
    if agent.verbose_logging:
        ...
        # on the fact that mangaba_logging.setup_logging() does not
        ...
        # handler level inside mangaba_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from mangaba_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from mangaba_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `mangaba model` to "
                        "select a provider, or run `mangaba setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff MANGABA_KANBAN_TASK is set).
    ...
    os.environ["MANGABA_SESSION_ID"] = agent.session_id
    try:
    ...
    # Session logs go into ~/.mangaba/sessions/ alongside gateway sessions
    mangaba_home = get_mangaba_home()
    agent.logs_dir = mangaba_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.mangaba/sessions/session_{sid}.json)
    ...
    try:
        from mangaba_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from mangaba_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "mangaba"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from mangaba_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from mangaba_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Mangaba Agent.  Choose a model with at least "
