"""Unified removal contract for every credential source Mangaba reads from.

Mangaba seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.mangaba/.env
    claude_code   — ~/.claude/.credentials.json
    mangaba_pkce   — ~/.mangaba/.anthropic_oauth.json
    ...
    manual        — user ran `mangaba auth add`
...
unify here is **removal**:
    ...
    ``mangaba auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), mangaba_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.mangaba/.env  → clear it
    ...
    """
    from mangaba_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.mangaba/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Mangaba.",
            f"  The pool entry is now suppressed — Mangaba will ignore "
            f"{env_var} until you run `mangaba auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Mangaba stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `mangaba auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_mangaba_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Mangaba Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `mangaba login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `mangaba auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``mangaba auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``mangaba auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `mangaba model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Mangaba auth store is not enough — _seed_from_singletons() would
    ...
    via ``mangaba auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `mangaba auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `mangaba auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Mangaba stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `mangaba auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="mangaba_pkce",
        remove_fn=_remove_mangaba_pkce,
        description="~/.mangaba/.anthropic_oauth.json",
    ))
    register(RemovalStep(
