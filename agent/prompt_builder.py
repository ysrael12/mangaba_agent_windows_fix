...
_MANGABA_MD_NAMES = (".mangaba.md", "MANGABA.md")


def _find_mangaba_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.mangaba.md`` or ``MANGABA.md``.
...
MANGABA_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Mangaba Agent "
    "itself, load the `mangaba-agent` skill with skill_view(name='mangaba-agent') "
    "before answering. Docs: https://dheiver2.github.io/mangaba-agent/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.mangaba/kanban.db`. Your task id is in "
    "`$MANGABA_KANBAN_TASK`; your workspace is `$MANGABA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $MANGABA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `mangaba kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Mangaba WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Mangaba itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Mangaba runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Mangaba itself is running. The host OS, home, and cwd "
                f"of the Mangaba process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Mangaba "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_mangaba_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.mangaba/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("MANGABA_PLATFORM")
        or get_session_env("MANGABA_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Mangaba Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `mangaba-agent` skill "
            "first. It has the actual commands (e.g. `mangaba config set …`, `mangaba tools`, "
            "`mangaba setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: mangaba setup, mangaba setup tools, mangaba setup terminal, mangaba status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from MANGABA_HOME and return its content, or None.
    ...
    """
    try:
        from mangaba_cli.config import ensure_mangaba_home
        ensure_mangaba_home()
    except Exception as e:
        logger.debug("Could not ensure MANGABA_HOME before loading SOUL.md: %s", e)
...
def _load_mangaba_md(cwd_path: Path) -> str:
    """.mangaba.md / MANGABA.md — walk to git root."""
    mangaba_md_path = _find_mangaba_md(cwd_path)
    if not mangaba_md_path:
        return ""
    try:
        content = mangaba_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = mangaba_md_path.name
        try:
            rel = str(mangaba_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".mangaba.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", mangaba_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .mangaba.md / MANGABA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from MANGABA_HOME is independent and always included when present.
    ...
    # SOUL.md from MANGABA_HOME only — skip when already loaded as identity
    if not skip_soul:
