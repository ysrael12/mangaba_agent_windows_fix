...
def _load_env() -> None:
    """Load .env from MANGABA_HOME (default ``~/.mangaba``)."""
    from mangaba_cli.env_loader import load_mangaba_dotenv

    mangaba_home = get_mangaba_home()
    loaded = load_mangaba_dotenv(mangaba_home=mangaba_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", mangaba_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="mangaba-acp",
        description="Run Mangaba Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Mangaba version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(mangaba_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import MangabaACPAgent  # noqa: F401

    print("Mangaba ACP check OK")


def _run_setup() -> None:
    from mangaba_cli.main import main as mangaba_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "mangaba", "model"]
        mangaba_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``mangaba postinstall`` and the runtime lazy installer.
