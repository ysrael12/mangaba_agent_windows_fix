"""ACP auth helpers — detect and advertise Mangaba authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "mangaba-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Mangaba runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Mangaba can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Mangaba.
    ...
    not have Mangaba provider credentials configured yet, so Mangaba always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Mangaba using the currently configured "
