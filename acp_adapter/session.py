"""ACP session manager — maps ACP sessions to Mangaba AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.mangaba/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Mangaba itself is running in WSL.

    Windows ACP clients can launch ``mangaba acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Mangaba from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["mangaba-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Mangaba agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Mangaba AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Mangaba runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.mangaba/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``MANGABA_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_mangaba_home``).
        """
