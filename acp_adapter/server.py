"""ACP agent server — exposes Mangaba Agent via the Agent Client Protocol."""
...
try:
    from mangaba_cli import __version__ as MANGABA_VERSION
except Exception:
    MANGABA_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Mangaba running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Mangaba cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Mangaba/OpenAI-compatible user content payload."""
...
class MangabaACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Mangaba AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Mangaba version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Mangaba maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from mangaba_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from mangaba_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Mangaba estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Mangaba changes it."""
        if not self._conn:
            return
        try:
        ...
        # mangaba_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="mangaba-agent", version=MANGABA_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Mangaba' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Mangaba setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Mangaba
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Mangaba on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Mangaba' local kawaii waiting/status
        ...
        # not the executor's. Also set MANGABA_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue MANGABA_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind MANGABA_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("MANGABA_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("MANGABA_SESSION_ID")
            os.environ["MANGABA_SESSION_ID"] = session_id
            try:
                ...
                # Restore MANGABA_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("MANGABA_INTERACTIVE", None)
                else:
                    os.environ["MANGABA_INTERACTIVE"] = previous_interactive
                # Restore MANGABA_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("MANGABA_SESSION_ID", None)
                else:
                    os.environ["MANGABA_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (MANGABA_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Mangaba Agent v{MANGABA_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Mangaba has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
