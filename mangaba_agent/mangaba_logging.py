"""Centralized logging setup for Mangaba Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.mangaba/logs/`` (profile-aware via ``get_mangaba_home()``).
...
from mangaba_agent.mangaba_constants import get_config_path, get_mangaba_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_mangaba_session_injector", False):
    ...
    _session_record_factory._mangaba_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``mangaba logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "mangaba_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Mangaba logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    mangaba_home
        Override for the Mangaba home directory.  Falls back to
        ``get_mangaba_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_mangaba_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._mangaba_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the mangaba group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from mangaba_cli.config import is_managed
