"""
Mangaba MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Mangaba-specific extra)

Usage:
    mangaba mcp serve
    mangaba mcp serve --verbose
...
logger = logging.getLogger("mangaba.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MANGABA_HOME."""
    try:
        from mangaba_agent.mangaba_constants import get_mangaba_home
        return get_mangaba_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MANGABA_HOME", Path.home() / ".mangaba")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from mangaba_agent.mangaba_constants import get_mangaba_home
        directory_file = get_mangaba_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MANGABA_HOME", Path.home() / ".mangaba")
...
class EventBridge:
    ...
    This is the Mangaba equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from mangaba_agent.mangaba_constants import get_mangaba_home
            db_file = get_mangaba_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MANGABA_HOME", Path.home() / ".mangaba")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Mangaba MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "mangaba",
        instructions=(
            "Mangaba Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Mangaba MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
