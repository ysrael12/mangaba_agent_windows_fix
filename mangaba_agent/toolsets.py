...
Usage:
    from mangaba_agent.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
_MANGABA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (MANGABA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `mangaba tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``mangaba tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (MANGABA_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Mangaba toolsets (CLI + messaging platforms)
    #
    ...
    "mangaba-cli": {
        ...
        "tools": _MANGABA_CORE_TOOLS,
        "includes": []
    },

    "mangaba-cron": {
        # Mirrors mangaba-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `mangaba tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as mangaba-cli; gated by `mangaba tools`",
        "tools": _MANGABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "mangaba-email": {
        "description": "Email bot toolset - interact with Mangaba via email (IMAP/SMTP)",
        "tools": _MANGABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "mangaba-sms": {
        "description": "SMS bot toolset - interact with Mangaba via SMS (Twilio)",
        "tools": _MANGABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "mangaba-gateway": {
        ...
        "tools": [],
        "includes": ["mangaba-telegram", "mangaba-discord", "mangaba-whatsapp", "mangaba-slack", "mangaba-signal", "mangaba-bluebubbles", "mangaba-homeassistant", "mangaba-email", "mangaba-sms", "mangaba-mattermost", "mangaba-matrix", "mangaba-dingtalk", "mangaba-feishu", "mangaba-wecom", "mangaba-wecom-callback", "mangaba-weixin", "mangaba-qqbot", "mangaba-webhook", "mangaba-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (mangaba-<name>).
        # Gives them _MANGABA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("mangaba-"):
            platform_name = name[len("mangaba-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_MANGABA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
