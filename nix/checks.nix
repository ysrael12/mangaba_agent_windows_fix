...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      mangaba-agent = self'.packages.default;
      mangabaVenv = mangaba-agent.mangabaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "mangaba-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${mangabaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "mangaba-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "mangaba-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${mangaba-agent}/bin/mangaba || (echo "FAIL: mangaba binary missing"; exit 1)
          test -x ${mangaba-agent}/bin/mangaba-agent || (echo "FAIL: mangaba-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${mangaba-agent}/bin/mangaba version 2>&1 | grep -qi "mangaba" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "mangaba-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in mangaba mangaba-agent mangaba-acp; do
            test -x ${mangaba-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "mangaba-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking mangaba --help ==="
          ${mangaba-agent}/bin/mangaba --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${mangaba-agent}/bin/mangaba --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "mangaba-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${mangaba-agent}/share/mangaba-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${mangaba-agent}/share/mangaba-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MANGABA_BUNDLED_SKILLS" ${mangaba-agent}/bin/mangaba || \
            (echo "FAIL: MANGABA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MANGABA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "mangaba-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${mangaba-agent}/share/mangaba-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${mangaba-agent}/share/mangaba-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MANGABA_BUNDLED_PLUGINS" ${mangaba-agent}/bin/mangaba || \
            (echo "FAIL: MANGABA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MANGABA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "mangaba-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${mangaba-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${mangaba-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MANGABA_TUI_DIR" ${mangaba-agent}/bin/mangaba || \
            (echo "FAIL: MANGABA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MANGABA_TUI_DIR set in wrapper"
        ...
        # Verify MANGABA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        mangaba-node = pkgs.runCommand "mangaba-node-version" { } ''
          set -e
          echo "=== Checking MANGABA_NODE in wrapper ==="
          grep -q "MANGABA_NODE" ${mangaba-agent}/bin/mangaba || \
            (echo "FAIL: MANGABA_NODE not set in wrapper"; exit 1)
          echo "PASS: MANGABA_NODE present in wrapper"

          MANGABA_NODE=$(sed -n "s/^export MANGABA_NODE='\(.*\)'/\1/p" ${mangaba-agent}/bin/mangaba)
          test -x "$MANGABA_NODE" || (echo "FAIL: MANGABA_NODE=$MANGABA_NODE not executable"; exit 1)
          echo "PASS: MANGABA_NODE executable at $MANGABA_NODE"

          NODE_MAJOR=$("$MANGABA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All MANGABA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MANGABA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "mangaba-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MANGABA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MANGABA_MANAGED guards ==="
          check_blocked "config set" ${mangaba-agent}/bin/mangaba config set model foo
          check_blocked "config edit" ${mangaba-agent}/bin/mangaba config edit
        ...
        extra-python-packages = let
          ...
          mangabaWithExtra = mangaba-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "mangaba-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${mangabaWithExtra}/bin/mangaba || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${mangabaWithExtra}/bin/mangaba || \
          ...
          if grep -q "PYTHONPATH" ${mangaba-agent}/bin/mangaba; then
        ...
        extra-dependency-groups = let
          mangabaWithGroups = mangaba-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "mangaba-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${mangabaWithGroups}"
          echo "venv: ${mangabaWithGroups.mangabaVenv}"
        ...
        in pkgs.runCommand "mangaba-config-roundtrip" {
          ...
          merge_and_load() {
            local mangaba_home="$1"
            export MANGABA_HOME="$mangaba_home"
            ${configMergeScript} ${nixSettings} "$mangaba_home/config.yaml"
            ${mangabaVenv}/bin/python3 -c '
import json, sys
