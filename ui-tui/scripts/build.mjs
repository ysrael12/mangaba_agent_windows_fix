...
await build({
  ...
  // Skip the prebuilt @mangaba/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@mangaba/ink': resolve(root, 'packages/mangaba-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The mangaba_cli launcher
