...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MANGABA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Mangaba on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MANGABA_TUI_TRUECOLOR=1.
