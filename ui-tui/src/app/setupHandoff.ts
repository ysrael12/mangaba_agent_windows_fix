import type { RunExternalProcess } from '@mangaba/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`mangaba ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching mangaba: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`mangaba ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
