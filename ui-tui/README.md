...
React + Ink terminal UI for Mangaba. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MANGABA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.mangaba/.mangaba_history` or under `MANGABA_HOME`.
...
```text
ui-tui/
  packages/mangaba-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      mangaba-ink.d.ts    type declarations for @mangaba/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent MangabaCLI subprocess for slash commands
```
