 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `mangaba_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "mangaba-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-mangaba-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-mangaba-theme-font", "true");
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [themeName, setThemeName] = useState<string>(() => {
    ...
    return window.localStorage.getItem(STORAGE_KEY) ?? "enterprise";
  });
