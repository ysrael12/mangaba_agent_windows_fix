# Mangaba Agent — Web UI

Browser-based dashboard for managing Mangaba Agent configuration, API keys, and monitoring active sessions.
...
`mangaba dashboard` on port 9119 serves the **built** bundle from `mangaba_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../mangaba_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from `@dheiver2/ui`'s `globals.css`:
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Mangaba Agent” — mixed case, no Mondwest, no `text-display` |
