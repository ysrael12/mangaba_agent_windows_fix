...
mangaba kanban watch --tenant <project-slug>

# Snapshot of the board
mangaba kanban list --tenant <project-slug>
mangaba kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
mangaba kanban stats --tenant <project-slug>

# Visual dashboard (browser)
mangaba dashboard

# Inspect a specific task (includes comments + events)
mangaba kanban show <task-id>

# Follow a single task's event stream
mangaba kanban tail <task-id>
```

Verify available subcommands with `mangaba kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `mangaba kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `mangaba kanban block <id>` then `mangaba kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `mangaba kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `mangaba kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `mangaba profile list` (profile exists?), `mangaba status` (gateway/dispatcher up?), and verify tenant. |
...
mangaba kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
mangaba kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(mangaba kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
mangaba kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
mangaba kanban block <task-id>
mangaba kanban archive <task-id>

# Diagnose what happened
mangaba kanban show <task-id>      # task body, comments, recent events
mangaba kanban tail <task-id>      # follow the live event stream
mangaba kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `mangaba kanban create` for the director
...
while true; do
    clear
    mangaba kanban list --tenant <slug>
    echo "---"
    mangaba kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `mangaba kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `mangaba profile list` and start
