...
Maps each role archetype to the Mangaba skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public mangaba-agent
...
## Mangaba skills relevant to video production

### Visual / rendering skills (`mangaba-agent/skills/creative/`)
...
### Design / pre-production skills (`mangaba-agent/skills/creative/`)
...
### Audio / media skills (`mangaba-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`mangaba-agent/skills/devops/`)
...
These are **not** Mangaba skills but external CLIs / APIs that profiles invoke.
...
## Built-in Mangaba tools for media review

These are native Mangaba tools — not invoked via terminal but through their own
...
key is present in `~/.mangaba/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Mangaba profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.mangaba/.env` → environment variable.
...
When a new Mangaba-public video skill ships:
