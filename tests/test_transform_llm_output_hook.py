...
from mangaba_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(mangaba_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <mangaba_home>/plugins/<name> and opt it in."""
    plugin_dir = mangaba_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    mangaba_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        mangaba_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MANGABA_HOME", str(tmp_path / "mangaba_empty"))
