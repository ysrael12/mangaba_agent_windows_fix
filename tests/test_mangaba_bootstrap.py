"""Tests for mangaba_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Mangaba entry point
(mangaba, mangaba-agent, mangaba-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Mangaba entry point imports mangaba_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported mangaba_bootstrap module.
    ...
    import mangaba_agent.mangaba_bootstrap as mangaba_bootstrap  # noqa: WPS433
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Mangaba CLI
...
class TestEntryPointsImportBootstrap:
    """Every Mangaba entry point must import mangaba_bootstrap as its
    ...
    # Entry points that invoke Mangaba as a process.  Each one must
    # import mangaba_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "mangaba_cli/main.py",   # mangaba CLI (console_script)
        "run_agent.py",          # mangaba-agent (console_script)
        "acp_adapter/entry.py",  # mangaba-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import mangaba_bootstrap' and that
        ...
        half-finished ``mangaba update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``mangaba_bootstrap`` as a top-level module) leaves mangaba
        ...
        """
        # Resolve relative to the mangaba-agent repo root.  Tests live
        # at tests/test_mangaba_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # mangaba start even when mangaba_bootstrap hasn't been
        ...
        assert first_import_name == "mangaba_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'mangaba_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import mangaba_bootstrap' line to be the first import."
        )
