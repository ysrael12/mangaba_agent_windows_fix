...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("mangaba_cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_getpass_without_tui():
    ...
    with patch("mangaba_cli.callbacks.getpass.getpass", return_value="secret-value"), patch(
        "mangaba_cli.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("mangaba_cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "mangaba_cli.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
...
def test_cli_chat_registers_secret_capture_callback():
    ...
    with patch("mangaba_agent.cli.get_tool_definitions", return_value=[]), patch.dict(
        "os.environ", {"LLM_MODEL": "", "MANGABA_MAX_ITERATIONS": ""}, clear=False
