...
minimal ``MangabaCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def mangaba_home(tmp_path, monkeypatch):
    """Isolated MANGABA_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".mangaba"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("MANGABA_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves MANGABA_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, mangaba_home):
        ...
        with patch("mangaba_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, mangaba_home):
        ...
        with patch("mangaba_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, mangaba_home):
    ...
    def test_no_assistant_message_skipped(self, mangaba_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, mangaba_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, mangaba_home):
