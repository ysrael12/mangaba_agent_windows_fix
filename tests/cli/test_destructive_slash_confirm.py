"""Tests for cli.MangabaCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full MangabaCLI (which requires extensive setup).
"""
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(MangabaCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(MangabaCLI._get_slash_confirm_display_fragments, self_)()
