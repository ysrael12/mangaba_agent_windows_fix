...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("mangaba/mangaba-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned mangaba/* and pr-* branches."""

    def test_prunes_orphaned_mangaba_branch(self, git_repo):
        """mangaba/mangaba-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "mangaba/mangaba-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "mangaba/mangaba-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "mangaba/mangaba-deadbeef" in result.stdout
        ...
        assert "mangaba/mangaba-deadbeef" in orphaned
        ...
        assert "mangaba/mangaba-deadbeef" not in result.stdout
