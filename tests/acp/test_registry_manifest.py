"""Tests for ACP Registry metadata shipped with Mangaba."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "mangaba-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Mangaba Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/dheiver2/mangaba-agent"
    assert data["website"].startswith("https://dheiver2.github.io/mangaba-agent/")
    assert data["authors"] == ["Dheiver Santos"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"mangaba-agent[acp]=={data['version']}"
    assert uvx["args"] == ["mangaba-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"mangaba-agent[acp]=={_pyproject_version()}"
    )
