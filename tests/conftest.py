"""Shared fixtures for the mangaba-agent test suite.
...
2. **Isolated MANGABA_HOME.** MANGABA_HOME points to a per-test tempdir so
   code reading ``~/.mangaba/*`` via ``get_mangaba_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".mangaba"`` instead of the canonical
   ``get_mangaba_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MANGABA_SESSION_* inheritance** — the agent's current gateway
...
# MANGABA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MANGABA_BEHAVIORAL_VARS = frozenset({
    "MANGABA_YOLO_MODE",
    "MANGABA_INTERACTIVE",
    "MANGABA_QUIET",
    ...
    "MANGABA_SESSION_CHAT_ID",
    "MANGABA_SESSION_CHAT_NAME",
    "MANGABA_SESSION_THREAD_ID",
    "MANGABA_SESSION_SOURCE",
    "MANGABA_SESSION_KEY",
    "MANGABA_GATEWAY_SESSION",
    "MANGABA_PLATFORM",
    "MANGABA_MODEL",
    "MANGABA_INFERENCE_MODEL",
    "MANGABA_INFERENCE_PROVIDER",
    "MANGABA_TUI_PROVIDER",
    "MANGABA_MANAGED",
    "MANGABA_DEV",
    "MANGABA_CONTAINER",
    "MANGABA_EPHEMERAL_SYSTEM_PROMPT",
    "MANGABA_TIMEZONE",
    "MANGABA_REDACT_SECRETS",
    "MANGABA_BACKGROUND_NOTIFICATIONS",
    "MANGABA_EXEC_ASK",
    "MANGABA_HOME_MODE",
    "MANGABA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.mangaba/kanban.db instead of the per-test MANGABA_HOME.
    "MANGABA_KANBAN_DB",
    "MANGABA_KANBAN_BOARD",
    "MANGABA_KANBAN_HOME",
    "MANGABA_KANBAN_WORKSPACES_ROOT",
    "MANGABA_KANBAN_LOGS_ROOT",
    "MANGABA_KANBAN_TASK",
    "MANGABA_KANBAN_WORKSPACE",
    "MANGABA_KANBAN_RUN_ID",
    "MANGABA_KANBAN_CLAIM_LOCK",
    "MANGABA_KANBAN_DISPATCH_IN_GATEWAY",
    "MANGABA_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MANGABA_HOME to per-test tempdirs so code that
    reads ``~/.mangaba/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MANGABA_* vars that could change test semantics.
    for name in _MANGABA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect MANGABA_HOME to a per-test tempdir. Code that reads
    #    ``~/.mangaba/*`` via ``get_mangaba_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.mangaba/*`` via
    #    ``Path.home() / ".mangaba"`` instead of ``get_mangaba_home()``
    #    is a bug to fix at the callsite.
    fake_mangaba_home = tmp_path / "mangaba_test"
    fake_mangaba_home.mkdir()
    (fake_mangaba_home / "sessions").mkdir()
    (fake_mangaba_home / "cron").mkdir()
    (fake_mangaba_home / "memories").mkdir()
    (fake_mangaba_home / "skills").mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(fake_mangaba_home))
    ...
    #    ~/.mangaba/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import mangaba_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_mangaba_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal mangaba config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``mangaba-gateway`` process
...
# correlated with a ``tests/mangaba_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> mangaba-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart mangaba-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting mangaba/python patterns are also blocked.
    """
    ...
    def _matches_mangaba_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MANGABA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_mangaba_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch mangaba-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m mangaba_cli.main".
                if (
                    "mangaba" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live mangaba-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting mangaba/python could hit the live gateway. "
