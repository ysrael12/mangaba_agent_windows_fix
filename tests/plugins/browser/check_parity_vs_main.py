...
Run from the PR worktree:
    ...
    cd ~/.mangaba/mangaba-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.mangaba/mangaba-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.mangaba/mangaba-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a mangaba-agent checkout"
)
...
# Isolated MANGABA_HOME for the config write.
