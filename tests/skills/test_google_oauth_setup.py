...
class TestMangabaConstantsFallback:
    """Tests for _mangaba_home.py fallback when mangaba_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_mangaba_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _mangaba_home.py with mangaba_constants blocked."""
        monkeypatch.setitem(sys.modules, "mangaba_constants", None)
        spec = importlib.util.spec_from_file_location("_mangaba_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_mangaba_home_env_var(self, monkeypatch, tmp_path):
        """When mangaba_constants is missing, MANGABA_HOME comes from env var."""
        monkeypatch.setenv("MANGABA_HOME", str(tmp_path / "custom-mangaba"))
        module = self._load_helper(monkeypatch)
        assert module.get_mangaba_home() == tmp_path / "custom-mangaba"

    def test_fallback_defaults_to_dot_mangaba(self, monkeypatch):
        """When mangaba_constants is missing and MANGABA_HOME unset, default to ~/.mangaba."""
        monkeypatch.delenv("MANGABA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_mangaba_home() == Path.home() / ".mangaba"

    def test_fallback_ignores_empty_mangaba_home(self, monkeypatch):
        """Empty/whitespace MANGABA_HOME is treated as unset."""
        monkeypatch.setenv("MANGABA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_mangaba_home() == Path.home() / ".mangaba"

    def test_fallback_display_mangaba_home_shortens_path(self, monkeypatch):
        """Fallback display_mangaba_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("MANGABA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_mangaba_home() == "~/.mangaba"

    def test_fallback_display_mangaba_home_profile_path(self, monkeypatch):
        """Fallback display_mangaba_home() handles profile paths under ~/."""
        monkeypatch.setenv("MANGABA_HOME", str(Path.home() / ".mangaba/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_mangaba_home() == "~/.mangaba/profiles/coder"

    def test_fallback_display_mangaba_home_custom_path(self, monkeypatch):
        """Fallback display_mangaba_home() returns full path for non-home locations."""
        monkeypatch.setenv("MANGABA_HOME", "/opt/mangaba-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_mangaba_home() == "/opt/mangaba-custom"

    def test_delegates_to_mangaba_constants_when_available(self):
        """When mangaba_constants IS importable, _mangaba_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_mangaba_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
