"""Tests for mangaba_constants module."""
...
class TestGetDefaultMangabaRoot:
    """Tests for get_default_mangaba_root() — Docker/custom deployment awareness."""

    def test_no_mangaba_home_returns_native(self, tmp_path, monkeypatch):
        """When MANGABA_HOME is not set, returns ~/.mangaba."""
        monkeypatch.delenv("MANGABA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_mangaba_root() == tmp_path / ".mangaba"

    def test_mangaba_home_is_native(self, tmp_path, monkeypatch):
        """When MANGABA_HOME = ~/.mangaba, returns ~/.mangaba."""
        native = tmp_path / ".mangaba"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MANGABA_HOME", str(native))
        assert get_default_mangaba_root() == native

    def test_mangaba_home_is_profile(self, tmp_path, monkeypatch):
        """When MANGABA_HOME is a profile under ~/.mangaba, returns ~/.mangaba."""
        native = tmp_path / ".mangaba"
    ...
    def test_mangaba_home_is_docker(self, tmp_path, monkeypatch):
        """When MANGABA_HOME points outside ~/.mangaba (Docker), returns MANGABA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MANGABA_HOME", str(docker_home))
        assert get_default_mangaba_root() == docker_home

    def test_mangaba_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any MANGABA_HOME outside ~/.mangaba is treated as the root."""
        custom = tmp_path / "my-mangaba-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MANGABA_HOME", str(custom))
        assert get_default_mangaba_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (MANGABA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MANGABA_HOME", str(profile))
        assert get_default_mangaba_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(mangaba_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(mangaba_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".mangaba"
