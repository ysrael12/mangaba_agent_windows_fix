...
MANGABA_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Mangaba root with a named profile and temp cron store."""
    root = tmp_path / "mangaba-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "mangaba profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets mangaba_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["profile_env_only_during_init"] = os.environ.get(
                    "MANGABA_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "MANGABA_PROFILE_TEST_SHARED"
                )
                observed["mangaba_home_during_init"] = str(get_mangaba_home())
                observed["scheduler_home_during_init"] = str(sched._get_mangaba_home())
            ...
            def run_conversation(self, *_a, **_kw):
                ...
                observed["mangaba_home_during_run"] = str(get_mangaba_home())
                observed["scheduler_home_during_run"] = str(sched._get_mangaba_home())
        ...
        from mangaba_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_mangaba_home", None)
        monkeypatch.setenv("MANGABA_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["mangaba_home_during_init"] == str(profile_home.resolve())
        assert observed["mangaba_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["MANGABA_HOME"] == str(root)
        assert sched._get_mangaba_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("MANGABA_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("MANGABA_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["MANGABA_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["MANGABA_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["MANGABA_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["MANGABA_PROFILE_TEST_SHARED"] == "outer"
        assert "MANGABA_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["MANGABA_CRON_TIMEOUT"] == "0"
        assert os.environ["MANGABA_HOME"] == str(root)
        assert sched._get_mangaba_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('MANGABA_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_mangaba_home", None)
    ...
    def test_run_job_without_profile_leaves_mangaba_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["mangaba_home_during_init"] == str(root)
        assert os.environ["MANGABA_HOME"] == str(root)
