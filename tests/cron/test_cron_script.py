...
- Path resolution (absolute, relative to MANGABA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MANGABA_HOME."""
    mangaba_home = tmp_path / ".mangaba"
    mangaba_home.mkdir()
    (mangaba_home / "cron").mkdir()
    (mangaba_home / "cron" / "output").mkdir()
    (mangaba_home / "scripts").mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MANGABA_DIR", mangaba_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", mangaba_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", mangaba_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", mangaba_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.mangaba/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MANGABA_SESSION_PLATFORM") is None
        assert os.environ.get("MANGABA_SESSION_CHAT_ID") is None
        assert os.environ.get("MANGABA_SESSION_CHAT_NAME") is None
