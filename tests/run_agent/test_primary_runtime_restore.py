...
class TestRestorePrimaryRuntime:
    ...
    def test_restores_model_and_provider(self):
        ...
        with patch("mangaba_agent.run_agent.OpenAI", return_value=MagicMock()):
            result = agent._restore_primary_runtime()
    ...
    def test_restore_survives_exception(self):
        ...
        with patch("mangaba_agent.run_agent.OpenAI", side_effect=Exception("connection refused")):
            result = agent._restore_primary_runtime()
...
class TestTryRecoverPrimaryTransport:
    ...
    def test_skipped_for_nous_provider(self):
        agent = _make_agent(provider="nous", base_url="https://inference.nous.dheiver2.com/v1")
        error = _make_transport_error("ReadTimeout")
    ...
    def test_survives_rebuild_failure(self):
        ...
        with patch("mangaba_agent.run_agent.OpenAI", side_effect=Exception("socket error")), \
             patch("time.sleep"):
            result = agent._try_recover_primary_transport(
                error, retry_count=3, max_retries=3,
            )
...
class TestRateLimitCooldown:
    ...
    def test_cooldown_set_on_rate_limit_reason(self):
        ...
        from mangaba_agent.run_agent import FailoverReason
        agent = _make_agent(
            fallback_model={"provider": "openrouter", "model": "anthropic/claude-sonnet-4"},
        )
