...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    mangaba_state = ModuleType("mangaba_state")
    mangaba_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "mangaba_state", mangaba_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Mangaba"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Mangaba"
