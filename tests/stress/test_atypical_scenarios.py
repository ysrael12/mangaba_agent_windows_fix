...
normal tests assume away:
  ...
  - Filesystem: MANGABA_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own MANGABA_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"mangaba_atyp_{name}_")
            os.environ["MANGABA_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("mangaba_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from mangaba_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "MANGABA_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from mangaba_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["MANGABA_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE mangaba_home: {resolved}")
...
@scenario("mangaba_home_with_spaces")
def _(home, kb):
    """MANGABA_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="mangaba with spaces ")
    os.environ["MANGABA_HOME"] = weird
    ...
    try:
        ...
        print(f"  MANGABA_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("mangaba_home_with_unicode")
def _(home, kb):
    """MANGABA_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/mangaba_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  MANGABA_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("mangaba_home_via_symlink")
def _(home, kb):
    """MANGABA_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() — two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="mangaba_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["MANGABA_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same MANGABA_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from mangaba_cli import web_server as ws  # noqa
    except Exception:
        pass
