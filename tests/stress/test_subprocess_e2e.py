...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves mangaba_cli.kanban_db on its own
  ...
  - worker logs are captured to MANGABA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "MANGABA_KANBAN_TASK": task.id,
            "MANGABA_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="mangaba_e2e_")
    os.environ["MANGABA_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from mangaba_cli import kanban_db as kb

    # Point the `mangaba` CLI child processes will run at the worktree
    # mangaba_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "mangaba")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m mangaba_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/mangaba_e2e_*/worker_*.log
    import glob
