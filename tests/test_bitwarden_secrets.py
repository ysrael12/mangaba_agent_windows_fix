...
binary download are exercised manually by `mangaba secrets bitwarden
...
def mangaba_home(tmp_path, monkeypatch):
    """Point Mangaba at an isolated home directory."""
...
def test_install_bws_happy_path(mangaba_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(mangaba_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(mangaba_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from mangaba_cli.env_loader import _apply_external_secret_sources
