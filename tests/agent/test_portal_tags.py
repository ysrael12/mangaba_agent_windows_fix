...
def test_mangaba_client_tag_includes_current_version():
    """The client tag must reflect mangaba_cli.__version__ verbatim."""
    from mangaba_cli import __version__
    from agent.portal_tags import mangaba_client_tag

    assert mangaba_client_tag() == f"client=mangaba-client-v{__version__}"


def test_mangaba_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import mangaba_client_tag

    tag = mangaba_client_tag()
    assert tag.startswith("client=mangaba-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import mangaba_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=mangaba-agent" in tags
    assert mangaba_client_tag() in tags
    assert len(tags) == 2
