"""Tests for MANGABA_HOME credential-file read blocking in file_safety.

Regression for https://github.com/dheiver2/mangaba-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``MANGABA_HOME`` itself,
...
for the credential stores while leaving arbitrary ``MANGABA_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_mangaba_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "mangaba_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_mangaba_home_path", lambda: home)
    return home
...
def test_arbitrary_mangaba_home_file_not_blocked(fake_home):
    """Non-credential files inside MANGABA_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Mangaba cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into MANGABA_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "mangaba_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at MANGABA_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == MANGABA_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at MANGABA_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in MANGABA_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_files_outside_mangaba_home_not_blocked(
    fake_home, tmp_path
):
    """A project's ``.env``, ``auth.json``, or ``mcp-tokens/`` outside
    MANGABA_HOME must remain readable — the gate is per-location, not
    per-filename."""
    from agent.file_safety import get_read_block_error
    ...
    for rel in (".env", "auth.json"):
        ...
        assert get_read_block_error(str(p)) is None, (
            f"{rel} outside MANGABA_HOME should NOT be blocked"
        )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, MANGABA_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_mangaba_home_path", lambda: profile)
    monkeypatch.setattr(fs, "_mangaba_root_path", lambda: root)
