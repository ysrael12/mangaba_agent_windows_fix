...
""",
        ...
        from mangaba_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        mangaba_home = tmp_path / ".mangaba"
        mangaba_home.mkdir()
        (mangaba_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))
        from mangaba_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from mangaba_cli.plugins import PluginContext, PluginManifest, PluginManager
