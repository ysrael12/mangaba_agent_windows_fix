...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Mangaba passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("MANGABA_HOME", "/users/alice/.mangaba/profiles/backend-worker")
        monkeypatch.setenv("MANGABA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "MANGABA_KANBAN_DB",
            "/users/alice/.mangaba/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.mangaba/kanban/boards/smoke"]'
            in cmd
        )
