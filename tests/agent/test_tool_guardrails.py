...
def test_default_config_has_warnings_and_hard_stop_enabled():
...
def test_hard_stop_disabled_repeated_identical_failed_call_warns_without_blocking():
...
def test_default_repeated_identical_failed_call_blocks_after_threshold():
...
def test_hard_stop_disabled_same_tool_varying_args_warns_without_halting():
    controller = ToolCallGuardrailController(
        ToolCallGuardrailConfig(
            hard_stop_enabled=False, same_tool_failure_warn_after=2, same_tool_failure_halt_after=3
        )
    )
...
def test_hard_stop_disabled_idempotent_no_progress_repeated_result_warns_without_blocking():
    controller = ToolCallGuardrailController(
