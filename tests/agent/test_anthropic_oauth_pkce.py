...
History:
  - PR #1775 first fixed this on ``run_mangaba_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_mangaba_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_mangaba_oauth_login_pure()`` run
...
def test_authorization_url_state_is_not_pkce_verifier(monkeypatch, tmp_path):
    ...
    from agent.anthropic_adapter import run_mangaba_oauth_login_pure

    result = run_mangaba_oauth_login_pure()
