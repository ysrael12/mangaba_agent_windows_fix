...
def backup_env(monkeypatch, tmp_path):
    """Isolate MANGABA_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_mangaba_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd mangaba_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under MANGABA_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level MANGABA_DIR picks up the tmp HOME."""
    import mangaba_constants
    importlib.reload(mangaba_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.mangaba/cron/jobs.json
