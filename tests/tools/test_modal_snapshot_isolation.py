...
def _restore_tool_modules():
    original_mangaba_home = os.environ.get("MANGABA_HOME")
    original_modules = {
        name: module
        ...
        or name == "mangaba_cli"
        or name.startswith("mangaba_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_mangaba_home is None:
            os.environ.pop("MANGABA_HOME", None)
        else:
            os.environ["MANGABA_HOME"] = original_mangaba_home
        _reset_modules(("tools", "mangaba_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    mangaba_cli = types.ModuleType("mangaba_cli")
    mangaba_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["mangaba_cli"] = mangaba_cli
    mangaba_home = tmp_path / "mangaba-home"
    os.environ["MANGABA_HOME"] = str(mangaba_home)
    sys.modules["mangaba_cli.config"] = types.SimpleNamespace(
        get_mangaba_home=lambda: mangaba_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="mangaba-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": mangaba_home / "modal_snapshots.json",
