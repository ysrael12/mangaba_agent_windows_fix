...
def _clean_state():
    ...
    saved = {}
    for k in ("MANGABA_INTERACTIVE", "MANGABA_GATEWAY_SESSION", "MANGABA_EXEC_ASK", "MANGABA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["MANGABA_INTERACTIVE"] = "1"
        session_key = os.getenv("MANGABA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No MANGABA_INTERACTIVE or MANGABA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
