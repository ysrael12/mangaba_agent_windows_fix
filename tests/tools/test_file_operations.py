...
class TestIsWriteDenied:
    ...
    def test_control_files_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.mangaba/{auth.json,config.yaml,webhook_subscriptions.json}
        writable — the same gap PR #15981 fixed for .env.
        """
        # Simulate a profile-mode MANGABA_HOME layout:
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__MANGABA_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__MANGABA_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "MANGABA_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__MANGABA_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__MANGABA_FENCE_a9f7b3__\n"
        )
