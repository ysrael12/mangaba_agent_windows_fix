...
def test_mangaba_provider_subclass_exists():
    """MangabaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _MANGABA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _MANGABA_PROVIDER_CLS is not None
    assert issubclass(_MANGABA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_mangaba_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns MangabaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _MANGABA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("MANGABA_HOME", str(tmp_path))
    ...
    assert _MANGABA_PROVIDER_CLS is not None
    assert isinstance(provider, _MANGABA_PROVIDER_CLS)
    assert provider._mangaba_server_name == "srv"
