...
TTS provider tools must resolve API keys from ``~/.mangaba/.env`` (via
``mangaba_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``mangaba_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.mangaba/.env`` carries the
    ...
    ``mangaba_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``mangaba_cli.config.load_env`` to simulate ``~/.mangaba/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while mangaba_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.mangaba/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from mangaba_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.mangaba/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
