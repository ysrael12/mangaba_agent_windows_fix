...
* ``mangaba_constants.get_mangaba_dir`` (cache path)
...
def tmp_cache_dir(tmp_path):
    """Override get_mangaba_dir so cache writes land under tmp_path."""
    ...
    with patch("mangaba_agent.mangaba_constants.get_mangaba_dir", _fake_get):
        yield cache_dir
...
class TestCaptureResponseRoutedToAuxVision:
    ...
    def test_som_capture_returns_text_with_vision_analysis(
        ...
        with patch.object(cu_tool, "_should_route_through_aux_vision",
                          return_value=True), \
             patch("mangaba_agent.model_tools._run_async", side_effect=_fake_run_async), \
