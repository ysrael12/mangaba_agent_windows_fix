...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._mangaba_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_mangaba_bin(self):
        """After _INSTALL_FAILED, manual install in MANGABA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        mangaba_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(mangaba_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(mangaba_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._mangaba_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == mangaba_bin
            assert _tirith_mod._resolved_path == mangaba_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# MANGABA_HOME isolation
# ---------------------------------------------------------------------------

class TestMangabaHomeIsolation:
    def test_mangaba_bin_dir_respects_mangaba_home(self):
        """_mangaba_bin_dir must use MANGABA_HOME, not hardcoded ~/.mangaba."""
        from tools.tirith_security import _mangaba_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MANGABA_HOME": tmpdir}):
            result = _mangaba_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_mangaba_home(self):
        """_failure_marker_path must use MANGABA_HOME, not hardcoded ~/.mangaba."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MANGABA_HOME": "/custom/mangaba"}):
            result = _failure_marker_path()
        assert result == "/custom/mangaba/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MANGABA_HOME; verify it's active."""
        mangaba_home = os.getenv("MANGABA_HOME")
        assert mangaba_home is not None, "MANGABA_HOME should be set by conftest"
        assert "mangaba_test" in mangaba_home, "Should point to test temp dir"

    def test_get_mangaba_home_fallback(self):
        """Without MANGABA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_mangaba_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MANGABA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".mangaba")
            result = _get_mangaba_home()
