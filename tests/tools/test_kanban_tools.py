...
Verifies:
  - Tools are gated on MANGABA_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `mangaba chat` sessions (no MANGABA_KANBAN_TASK) must have
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("MANGABA_KANBAN_TASK", "t_fake")
    home = tmp_path / ".mangaba"
    home.mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the MANGABA_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: MANGABA_HOME isolated, MANGABA_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".mangaba"
    home.mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(home))
    monkeypatch.setenv("MANGABA_PROFILE", "test-worker")
    monkeypatch.delenv("MANGABA_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("MANGABA_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["mangaba_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("MANGABA_SESSION_ID", "session-trusted")
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to MANGABA_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``MANGABA_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``mangaba-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "mangaba-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from MANGABA_PROFILE in the fixture, not the
        # caller-supplied "mangaba-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("MANGABA_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from mangaba_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no MANGABA_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (MANGABA_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has MANGABA_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no MANGABA_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # MANGABA_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("MANGABA_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("MANGABA_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no MANGABA_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via MANGABA_KANBAN_BOARD env var,
...
# restarting Mangaba. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Mangaba home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    MANGABA_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".mangaba"
    home.mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(home))
    # Make sure neither MANGABA_KANBAN_DB nor MANGABA_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("MANGABA_KANBAN_DB", raising=False)
    monkeypatch.delenv("MANGABA_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("MANGABA_KANBAN_TASK", raising=False)
    monkeypatch.setenv("MANGABA_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning MANGABA_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".mangaba"
    home.mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(home))
    monkeypatch.setenv("MANGABA_PROFILE", "alt-worker")
    monkeypatch.delenv("MANGABA_KANBAN_DB", raising=False)
    monkeypatch.delenv("MANGABA_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
