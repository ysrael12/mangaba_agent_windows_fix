...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because mangaba uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("MANGABA_GATEWAY_SESSION", "1")
        monkeypatch.delenv("MANGABA_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("MANGABA_INTERACTIVE", raising=False)
        monkeypatch.delenv("MANGABA_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("MANGABA_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("MANGABA_INTERACTIVE", false_like_value)
        monkeypatch.delenv("MANGABA_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("MANGABA_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["MANGABA_INTERACTIVE", "MANGABA_GATEWAY_SESSION", "MANGABA_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("MANGABA_INTERACTIVE", "MANGABA_GATEWAY_SESSION", "MANGABA_EXEC_ASK"):
