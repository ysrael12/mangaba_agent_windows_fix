...
def server():
    with patch.dict(
        "sys.modules",
        {
            "mangaba_constants": MagicMock(
                get_mangaba_home=MagicMock(return_value="/tmp/mangaba_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'mangaba-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'mangaba-release' patched"
    }
