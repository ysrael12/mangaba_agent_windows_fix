...
`mangaba tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.dheiver2.com during one render. The cache is keyed
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test MANGABA_HOME.
...
def test_get_nous_auth_status_caches_failure_path(tmp_path, monkeypatch):
    ...
    Teknium's case: ~31 cache misses per `mangaba tools` "All Platforms"
