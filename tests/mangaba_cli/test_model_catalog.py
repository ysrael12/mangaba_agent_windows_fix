"""Tests for mangaba_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate MANGABA_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from mangaba_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from mangaba_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in mangaba_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from mangaba_cli import model_catalog
        from mangaba_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from mangaba_cli import model_catalog
        from mangaba_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        ...
        the slash-command picker until the next Mangaba release.
        """
        ...
        # ``MANGABA_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` MANGABA_HOME directly instead.
        import importlib
        from mangaba_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from mangaba_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["MANGABA_HOME"])
