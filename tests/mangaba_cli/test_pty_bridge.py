"""Unit tests for mangaba_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf mangaba-ok"])
        try:
            output = _read_until(bridge, b"mangaba-ok")
            assert b"mangaba-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$MANGABA_PTY_TEST\""],
            env={**os.environ, "MANGABA_PTY_TEST": "pty-env-works"},
        )
        try:
