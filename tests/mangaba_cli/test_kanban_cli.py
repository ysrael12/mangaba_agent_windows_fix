"""Tests for the kanban CLI surface (mangaba_cli.kanban)."""
...
from mangaba_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`mangaba kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from mangaba_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from mangaba_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("MANGABA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("MANGABA_KANBAN_BOARD") == "beta"
