"""Tests for the update check mechanism in mangaba_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "mangaba-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("mangaba_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MANGABA_HOME", str(tmp_path))
    with patch("mangaba_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "mangaba_cli" / "banner.py"
    ...
    monkeypatch.setenv("MANGABA_HOME", str(tmp_path))
    with patch("mangaba_cli.banner.subprocess.run") as mock_run:
        with patch("mangaba_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when MANGABA_HOME has no git repo."""
    import mangaba_cli.banner as banner
    ...
    # Point MANGABA_HOME at a temp dir with no mangaba-agent/.git
    monkeypatch.setenv("MANGABA_HOME", str(tmp_path))
    with patch("mangaba_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from mangaba_cli.main import _invalidate_update_cache

    # Build a fake ~/.mangaba with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"MANGABA_HOME": str(default_home)}):
