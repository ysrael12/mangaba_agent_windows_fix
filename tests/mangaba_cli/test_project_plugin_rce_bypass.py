...
1. ``mangaba_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.mangaba/plugins/`` source via
   ``os.environ.get("MANGABA_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``mangaba_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — MANGABA_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.mangaba/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("MANGABA_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("MANGABA_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("MANGABA_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"MANGABA_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``MANGABA_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MANGABA_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "mangaba_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "mangaba_dashboard_plugin_evil" not in sys.modules
