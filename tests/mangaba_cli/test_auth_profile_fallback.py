...
When ``MANGABA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.mangaba/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.mangaba            (has its own auth.json fixture)
    * Profile     -> tmp_path/.mangaba/profiles/coder   (active, MANGABA_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (MANGABA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.mangaba as the "real home" — which is NOT equal
    # to the MANGABA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    mangaba_home = tmp_path / "classic"
    mangaba_home.mkdir()
    monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))

    _write(mangaba_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from mangaba_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: MANGABA_HOME is set to a custom path that is NOT under
    # ~/.mangaba/profiles/ — get_default_mangaba_root() returns MANGABA_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from mangaba_cli.auth import read_credential_pool, write_credential_pool
