...
The cache exists to keep `mangaba tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from mangaba_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("mangaba_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from mangaba_cli import config as config_mod
    from mangaba_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_mangaba_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "mangaba-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("mangaba_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
