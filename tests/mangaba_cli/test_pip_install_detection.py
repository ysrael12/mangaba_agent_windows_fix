...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("mangaba_cli.config.get_managed_system", return_value=None), \
         patch("mangaba_cli.config.get_mangaba_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When MANGABA_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("mangaba_cli.config.get_managed_system", return_value="NixOS"), \
         patch("mangaba_cli.config.get_mangaba_home", return_value=tmp_path):
        from mangaba_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from mangaba_cli.config import recommended_update_command_for_method
...
def test_docker_detected_via_dockerenv(tmp_path):
    with patch("mangaba_cli.config.get_managed_system", return_value=None), \
         patch("mangaba_cli.config.get_mangaba_home", return_value=tmp_path), \
         patch("mangaba_agent.mangaba_constants.is_container", return_value=True):
        from mangaba_cli.config import detect_install_method
        assert detect_install_method(project_root=tmp_path) == "docker"
