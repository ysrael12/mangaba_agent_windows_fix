"""Tests for IRC gateway configuration via `mangaba setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Mangaba install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Mangaba install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#mangaba")
            monkeypatch.setenv("IRC_NICKNAME", "mangaba-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `mangaba setup gateway` experience with IRC present."""
