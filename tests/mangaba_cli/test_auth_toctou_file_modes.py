"""Regression tests for TOCTOU-safe credential file writers in ``mangaba_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.mangaba/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks MANGABA_HOME via
    # get_default_mangaba_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("MANGABA_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from mangaba_cli import auth as auth_mod
