"""Tests for --ignore-user-config and --ignore-rules flags on `mangaba chat`.
...
files. In Mangaba the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.mangaba/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → MangabaCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("MANGABA_IGNORE_USER_CONFIG", "MANGABA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``MANGABA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<mangaba_home>/config.yaml`` is
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("MANGABA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``MangabaCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting MANGABA_IGNORE_RULES=1 flips MangabaCLI.ignore_rules True."""
        monkeypatch.setenv("MANGABA_IGNORE_RULES", "1")
        ...
        # Replicate the exact logic from cli.py MangabaCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``mangaba_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("MANGABA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("MANGABA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("MANGABA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("MANGABA_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("MANGABA_IGNORE_USER_CONFIG") == "1"
        assert "MANGABA_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "MANGABA_IGNORE_USER_CONFIG" not in os.environ
        assert "MANGABA_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="mangaba")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real mangaba parser accepts both flags."""
        ...
        import inspect
        import mangaba_cli.main as hm
        src = inspect.getsource(hm)
        assert "MANGABA_IGNORE_USER_CONFIG" in src
        assert "MANGABA_IGNORE_RULES" in src
