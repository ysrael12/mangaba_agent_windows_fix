...
Covers:
 - Normal path (tokens already in Mangaba auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Mangaba
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `mangaba auth openai-codex`. The old
...
@pytest.fixture()
def mangaba_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Mangaba auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(mangaba_auth_only_env):
    """openai-codex appears when tokens are already in Mangaba auth store."""
...
def test_codex_picker_uses_live_codex_catalog(mangaba_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Mangaba
    auth store."""
    mangaba_home = tmp_path / ".mangaba"
    mangaba_home.mkdir()
