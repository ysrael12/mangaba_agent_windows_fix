...
Before the fix, ``mangaba whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``mangaba gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    mangaba = home / ".mangaba"
    mangaba.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("MANGABA_HOME", str(mangaba))
...
def _env_value(mangaba_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("mangaba_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``mangaba whatsapp`` with an existing paired session and
