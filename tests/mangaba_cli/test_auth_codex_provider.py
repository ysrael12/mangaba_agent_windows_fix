"""Tests for Codex auth — tokens stored in Mangaba auth store (~/.mangaba/auth.json)."""
...
def _setup_mangaba_auth(mangaba_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Mangaba auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_mangaba_auth(mangaba_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_mangaba_auth(mangaba_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))
    ...
    monkeypatch.setattr("mangaba_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    mangaba_home = tmp_path / "mangaba"
    _setup_mangaba_auth(mangaba_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Mangaba auth store, not ~/.codex/."""
    mangaba_home = tmp_path / "mangaba"
    codex_home = tmp_path / "codex-cli"
    mangaba_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "mangaba-at", "refresh_token": "mangaba-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Mangaba store
    assert not (codex_home / "auth.json").exists()

    # Mangaba auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "mangaba-at"


def test_resolve_returns_mangaba_auth_store_source(tmp_path, monkeypatch):
    mangaba_home = tmp_path / "mangaba"
    _setup_mangaba_auth(mangaba_home)
    monkeypatch.setenv("MANGABA_HOME", str(mangaba_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "mangaba_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("mangaba_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("mangaba_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
