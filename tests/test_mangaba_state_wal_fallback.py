...
(SQLITE_PROTOCOL — typical on NFS/SMB), Mangaba must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``MANGABA_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    mangaba_state._set_last_init_error(None)
    yield
    mangaba_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    mangaba_state._wal_fallback_warned_paths.clear()
    yield
    mangaba_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see mangaba_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        mangaba_state._set_last_init_error("OperationalError: locking protocol")
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        mangaba_state._set_last_init_error("OperationalError: generic SQLite error")
