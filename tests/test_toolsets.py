...
class TestToolsetConsistency:
    ...
    def test_mangaba_platforms_share_core_tools(self):
        """All mangaba-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on mangaba-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["mangaba-cli", "mangaba-telegram", "mangaba-discord", "mangaba-whatsapp", "mangaba-slack", "mangaba-signal", "mangaba-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_mangaba_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("mangaba-whatsapp")

    def test_mangaba_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("mangaba-api-server")
