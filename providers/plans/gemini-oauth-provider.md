...
## Client ID
- Need to register a "Desktop app" OAuth client on a Dheiver Santos GCP project
...
## Token Lifecycle
- Store at `~/.mangaba/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `mangaba_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `mangaba_cli/models.py` — Add Gemini model catalog
4. `mangaba_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `mangaba_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `mangaba_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Dheiver Santos GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via MANGABA_GEMINI_CLIENT_ID env var
...
- mangaba-agent Copilot OAuth: `mangaba_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
