"""Profile distributions — shareable, packaged Mangaba profiles via git.

A distribution is a Mangaba profile published as a git repository (or
...
* ``mangaba profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``mangaba skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``mangaba profile``, not a parallel tree):

    mangaba profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "mangaba_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".mangaba_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "mangaba-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            mangaba_requires=str(data.get("mangaba_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.mangaba_requires:
            out["mangaba_requires"] = self.mangaba_requires
        if self.author:
...
def check_mangaba_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Mangaba {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``mangaba profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Mangaba profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Mangaba distribution."
        )

    # Version check up-front so we fail fast
    check_mangaba_requires(manifest.mangaba_requires, mangaba_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.mangaba).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="mangaba_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `mangaba profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`mangaba profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="mangaba_dist_update_") as tmp:
        plan = plan_install(
