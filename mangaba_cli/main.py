#!/usr/bin/env python3
"""
Mangaba CLI - Main entry point.

Usage:
    mangaba                     # Interactive chat (default)
    mangaba chat                # Interactive chat
    mangaba gateway             # Run gateway in foreground
    mangaba gateway start       # Start gateway as service
    mangaba gateway stop        # Stop gateway service
    mangaba gateway status      # Show gateway status
    mangaba gateway install     # Install gateway service
    mangaba gateway uninstall   # Uninstall gateway service
    ...
    mangaba cron list           # List cron jobs
    mangaba cron status         # Check if cron scheduler is running
    mangaba doctor              # Check configuration and dependencies
    mangaba honcho setup                    # Configure Honcho AI memory integration
    mangaba honcho status                   # Show Honcho config and connection status
    mangaba honcho sessions                 # List directory → session name mappings
    mangaba honcho map <name>               # Map current directory to a session name
    mangaba honcho peer                     # Show peer names and dialectic settings
    mangaba honcho peer --user NAME         # Set user peer name
    mangaba honcho peer --ai NAME           # Set AI peer name
    mangaba honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    mangaba honcho mode                     # Show current memory mode
    mangaba honcho mode [hybrid|honcho|local]  # Set memory mode
    mangaba honcho tokens                   # Show token budget settings
    mangaba honcho tokens --context N       # Set session.context() token cap
    mangaba honcho tokens --dialectic N     # Set dialectic result char cap
    mangaba honcho identity                 # Show AI peer identity representation
    mangaba honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    mangaba honcho migrate                  # Step-by-step migration guide: OpenClaw native → Mangaba + Honcho
    mangaba version             Show version
    mangaba update              Update to latest version
    mangaba uninstall           Uninstall Mangaba Agent
    mangaba acp                 Run as an ACP server for editor integration
    mangaba sessions browse     Interactive session picker with search

    mangaba claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: mangaba_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``mangaba_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``mangaba update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``mangaba_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, mangaba crashes on import and the user can't run
...
def _print_fast_version_info() -> None:
    from mangaba_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Mangaba Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``mangaba --version`` before config/logging imports on Termux."""
    if os.environ.get("MANGABA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to MANGABA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (mangaba tools, mangaba setup, mangaba model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'mangaba {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any mangaba module import.
#
# Many modules cache MANGABA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("MANGABA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.mangaba/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MANGABA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors mangaba_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If MANGABA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.mangaba/profiles/coder or
    # /opt/data/profiles/coder).  If MANGABA_HOME points to the mangaba root
    # instead (e.g. systemd hardcodes MANGABA_HOME=/root/.mangaba), we must
    # still read active_profile — the user may have switched profiles via
    # `mangaba profile use` and the gateway should honour that choice.
    # See issue #22502.
    mangaba_home_env = os.environ.get("MANGABA_HOME", "")
    if profile_name is None and mangaba_home_env:
        if Path(mangaba_home_env).parent.name == "profiles":
            return
    ...
    # 3. If we found a profile, resolve and set MANGABA_HOME
    if profile_name is not None:
        try:
            from mangaba_cli.profiles import resolve_profile_env

            mangaba_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent mangaba from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["MANGABA_HOME"] = mangaba_home
...
# Load .env from ~/.mangaba/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from mangaba_cli.config import get_mangaba_home
from mangaba_cli.env_loader import load_mangaba_dotenv

load_mangaba_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → MANGABA_REDACT_SECRETS env
# var BEFORE mangaba_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "MANGABA_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_mangaba_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["MANGABA_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_mangaba_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MANGABA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MANGABA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from mangaba_cli.config import get_env_path, get_mangaba_home, load_config
    from mangaba_cli.auth import get_auth_status

    # Determine whether Mangaba itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from mangaba_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_mangaba_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_mangaba_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Mangaba has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Mangaba to use their tokens.
    if _has_mangaba_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, mangaba_bin
        cli_args: the original CLI arguments (everything after 'mangaba')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    mangaba_bin = container_info["mangaba_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo mangaba {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo mangaba {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  mangaba --tui --resume {target}")
    if title:
        print(f'  mangaba --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @mangaba/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@mangaba" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/mangaba-ink/package.json",
    "packages/mangaba-ink/package-lock.json",
    "packages/mangaba-ink/index.js",
    "packages/mangaba-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``MANGABA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("MANGABA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``MANGABA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (MANGABA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("MANGABA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from mangaba_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("MANGABA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MANGABA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MANGABA_TUI_DIR (e.g. `unset MANGABA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("MANGABA_QUIET"):
    ...
    if tui_dev:
        # Keep the local @mangaba/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @mangaba/ink resolves through
        # packages/mangaba-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "mangaba-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from mangaba_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="mangaba-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["MANGABA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["MANGABA_PYTHON_SRC_ROOT"] = os.environ.get(
        "MANGABA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("MANGABA_PYTHON", sys.executable)
    env.setdefault("MANGABA_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["MANGABA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["MANGABA_MODEL"] = model
        env["MANGABA_INFERENCE_MODEL"] = model
    if provider:
        env["MANGABA_TUI_PROVIDER"] = provider
        env["MANGABA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["MANGABA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["MANGABA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["MANGABA_TUI_SKILLS"] = value
    if query:
        env["MANGABA_TUI_QUERY"] = query
    if image:
        env["MANGABA_TUI_IMAGE"] = image
    if checkpoints:
        env["MANGABA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["MANGABA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["MANGABA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["MANGABA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["MANGABA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["MANGABA_ACCEPT_HOOKS"] = "1"
    ...
    # MANGABA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `mangaba --tui` try to
    ...
    # still set MANGABA_TUI_RESUME themselves.
    env.pop("MANGABA_TUI_RESUME", None)
    if resume_session_id:
        env["MANGABA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `mangaba update` so
    ...
    if code == 42:
        from mangaba_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MANGABA_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``mangaba kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``mangaba kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("MANGABA_KANBAN_BOARD"):
        return
    try:
        from mangaba_cli.kanban_db import get_current_board

        os.environ["MANGABA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("MANGABA_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'mangaba sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from mangaba_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from mangaba_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'mangaba doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Mangaba isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  mangaba setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'mangaba setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from mangaba_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["MANGABA_YOLO_MODE"] = "1"
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["MANGABA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["MANGABA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from mangaba_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from mangaba_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from mangaba_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Mangaba?")
        print()
    ...
    # has no creds.json.  Every subsequent `mangaba gateway` then paid a 30s
    ...
    session_dir = get_mangaba_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: mangaba gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `mangaba gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  mangaba gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Mangaba Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  mangaba gateway")
        ...
        print()
        print("  Or install as a service: mangaba gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'mangaba whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from mangaba_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from mangaba_cli.config import stamp_install_method
    from mangaba_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Mangaba post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``mangaba model``) and the setup wizard
    ...
    from mangaba_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("MANGABA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'mangaba model' for "
                "available providers, or run 'mangaba doctor' to diagnose config "
                "issues."
            )
    ...
    from mangaba_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.mangaba/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.mangaba/.env if the active provider is not 'custom'.
    ...
    """
    from mangaba_cli.config import get_env_value, save_env_value, load_config
...
#
# Mangaba uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `mangaba model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `mangaba model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Mangaba only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``mangaba model`` flow, then route aux tasks to them here.
    """
    from mangaba_cli.config import load_config
    from mangaba_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from mangaba_cli.auth import _prompt_model_selection
    from mangaba_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from mangaba_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from mangaba_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.mangaba/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `mangaba setup` from scratch. OpenRouter
    ...
    from mangaba_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.mangaba/.env by hand.
    ...
    from mangaba_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from mangaba_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from mangaba_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from mangaba_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from mangaba_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `mangaba auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from mangaba_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from mangaba_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``mangaba model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``mangaba model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``mangaba auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from mangaba_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from mangaba_cli.auth import _save_model_choice, deactivate_provider
    from mangaba_cli.config import get_env_value, load_config, save_config
    ...
    from mangaba_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `mangaba model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from mangaba_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Mangaba URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from mangaba_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from mangaba_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from mangaba_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Mangaba will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Mangaba will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from mangaba_cli.curses_ui import flush_stdin
...
# it to the model-selection handlers so plain `mangaba --tui` does not pay for
...
def _model_flow_copilot(config, current_model=""):
    ...
    from mangaba_cli.config import save_env_value, load_config, save_config
    from mangaba_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from mangaba_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from mangaba_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from mangaba_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Mangaba turns to `copilot --acp`.")
    print("  Mangaba currently starts its own ACP subprocess for each request.")
    print("  Mangaba uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set MANGABA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``mangaba setup`` / ``mangaba model``.
    ...
    recover from a malformed paste without editing ``~/.mangaba/.env`` by hand.
    ...
    from mangaba_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `mangaba setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from mangaba_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so mangaba knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Mangaba typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Mangaba, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from mangaba_cli.models import fetch_lmstudio_models
        ...
        from mangaba_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from mangaba_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'mangaba model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Mangaba CLI with a provider."""
    from mangaba_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from mangaba_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from mangaba_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from mangaba_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from mangaba_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from mangaba_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from mangaba_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``mangaba slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: mangaba slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `mangaba slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from mangaba_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from mangaba_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from mangaba_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from mangaba_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from mangaba_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from mangaba_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from mangaba_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Mangaba home directory to a zip file."""
    if getattr(args, "quick", False):
        from mangaba_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from mangaba_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Mangaba backup from a zip file."""
    from mangaba_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from mangaba_cli.banner import check_for_updates
        from mangaba_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Mangaba Agent."""
    _require_tty("uninstall")
    from mangaba_cli.uninstall import run_uninstall
...
# Critical files that every ``mangaba`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``mangaba update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "mangaba_cli/main.py",
    "mangaba_cli/config.py",
    "mangaba_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "mangaba_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``mangaba`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="mangaba-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``mangaba update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``mangaba_cli/web_dist/`` (per vite.config.ts
    outDir: "../mangaba_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "mangaba_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``mangaba update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``mangaba web``).
    ...
    # build path usable on a stock `py -m mangaba_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``mangaba dashboard`` processes other than ourselves.

    ``mangaba dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``mangaba update`` replaces files on disk, the running
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "mangaba.*dashboard"`) keeps us consistent
            # with `mangaba_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `mangaba update`.
    ...
    print("  Preview now:  mangaba curator run --dry-run")
    print("  Pause it:     mangaba curator pause")
    print(
        "  Docs:         https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``mangaba update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``mangaba update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: mangaba curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``mangaba dashboard`` processes.

    Called at the end of ``mangaba update`` (default ``reason``) and also
    from ``mangaba dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    mangaba dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Mangaba Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/dheiver2/mangaba-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="mangaba-update-")
        zip_path = os.path.join(tmp_dir, f"mangaba-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to mangaba-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"mangaba-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "mangaba-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Mangaba behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes mangaba completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Mangaba couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Mangaba couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Mangaba behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `mangaba update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/dheiver2/mangaba-agent.git",
    "git@github.com:dheiver2/mangaba-agent.git",
    "https://github.com/dheiver2/mangaba-agent",
    "git@github.com:dheiver2/mangaba-agent",
}
OFFICIAL_REPO_URL = "https://github.com/dheiver2/mangaba-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Mangaba repository.")
        print("  This means you may miss updates from dheiver2/mangaba-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/dheiver2/mangaba-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/dheiver2/mangaba-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``mangaba update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``mangaba update`` is still progressing even if pip/uv itself is silent.
    """
...
def _mangaba_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "mangaba.exe",
        scripts_dir / "mangaba-gateway.exe",
    ]


def _detect_concurrent_mangaba_instances(
    ...
    The Mangaba Desktop Electron app spawns ``mangaba.EXE`` as a backend child,
    so during ``mangaba update`` the user-invoked process and the desktop's
    ...
    shims (``mangaba.exe`` / ``mangaba-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``mangaba update`` invocation never reports itself.
    ...
    for shim in _mangaba_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "mangaba.exe"
    lines = ["✗ Another mangaba.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Mangaba Desktop, exit any open `mangaba` REPLs, and")
    lines.append("  stop the gateway (`mangaba gateway stop`) before retrying.")
    lines.append("  Override with `mangaba update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_mangaba_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``mangaba.exe``.
    ...
    when ``mangaba update`` runs, ``mangaba.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``mangaba.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next mangaba invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Mangaba Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Mangaba Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _mangaba_exe_shims(scripts_dir):
        ...
        print(
            "    Close Mangaba Desktop, exit other `mangaba` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `mangaba update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_mangaba_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``mangaba.exe.old.*`` left by prior updates.

    Called early on every mangaba invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``mangaba update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `mangaba update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``mangaba.exe`` / ``mangaba-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_mangaba_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_mangaba_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.mangaba/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``mangaba update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.mangaba/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``mangaba update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``mangaba update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.mangaba/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``mangaba update --gateway``) the update is already
    ...
    try:
        ...
        # mangaba_cli.config.get_mangaba_home to simulate setup failure.
        from mangaba_cli.config import get_mangaba_home as _get_mangaba_home

        logs_dir = _get_mangaba_home() / "logs"
...
def _cmd_update_check():
    """Implement ``mangaba update --check``: fetch and report without installing."""
    from mangaba_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from mangaba_cli.config import recommended_update_command
        from mangaba_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``mangaba update`` without requiring a reinstall.  The
    ...
    ``mangaba`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/mangaba.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v mangaba'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/mangaba, code at /usr/local/lib/mangaba-agent).
    fhs_link = Path("/usr/local/bin/mangaba")
    ...
    path_comment = (
        "# Mangaba Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of MANGABA_HOME before running the update.
    ...
    MANGABA_HOME directories.  The ``--backup`` flag on ``mangaba update``
    ...
    try:
        from mangaba_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        ...
        home = get_mangaba_home()
        try:
            display_path = f"{display_mangaba_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  mangaba import {out_path}")
...
def cmd_update(args):
    """Update Mangaba Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from mangaba_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Mangaba Agent")
        return
...
def _cmd_update_pip(args):
    """Update Mangaba via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "mangaba-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "mangaba-agent"]
    ...
    print("✓ Update complete! Restart mangaba to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Mangaba Agent...")
    print()

    # On Windows, abort early if another mangaba.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_mangaba_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in mangaba_cli/config.py bricked
        # every user who ran ``mangaba update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``mangaba
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``mangaba update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_mangaba_home added to mangaba_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload mangaba_constants so that any lazy
        ...
        # seed_profile_skills() uses subprocess with an explicit MANGABA_HOME so
        # it is not affected by sync_skills()'s module-level MANGABA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's MANGABA_HOME env var points at the default or a named profile.
        try:
            from mangaba_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: mangaba config migrate")
            else:
                print()
                print("Skipped. Run 'mangaba config migrate' later to configure.")
        else:
        ...
        # check `mangaba curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``mangaba update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from mangaba_cli.tools_config import install_cua_driver
        ...
        # When running as ``mangaba update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_mangaba_home() / ".update_exit_code"
            try:
        ...
        try:
            from mangaba_cli.gateway import (
                is_macos,
            ...
            # Discover all mangaba-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. mangaba-gateway.service or mangaba-gateway-coder.service
                            ...
                            # path in `mangaba gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: mangaba gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: mangaba -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Mangaba gateway unit files are still installed.
        # When both mangaba.service (from a pre-rename install) and the
        # current mangaba-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `mangaba update` surfaces the issue until the user migrates.
        try:
            from mangaba_cli.gateway import (
                has_legacy_mangaba_units,
                _find_legacy_mangaba_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_mangaba_units():
                print()
                print("⚠ Legacy Mangaba gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_mangaba_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (mangaba.service) fight the current")
                print("  mangaba-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    mangaba gateway migrate-legacy")
                print()
        ...
        print("  mangaba model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``mangaba -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `mangaba profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_mangaba_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → mangaba -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.mangaba)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  mangaba profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     mangaba -p {name} chat")
                else:
        ...
        from mangaba_cli import profiles as _profiles_mod
        ...
        from mangaba_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `mangaba profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from mangaba_cli.profiles import profile_exists
        ...
        from mangaba_cli.profiles import rename_profile
        ...
        from mangaba_cli.profiles import export_profile
        ...
        from mangaba_cli.profiles import import_profile
        ...
        import tempfile
        from mangaba_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="mangaba_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  mangaba -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      mangaba -p {plan.manifest.name} chat")
        ...
        from mangaba_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `mangaba profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"mangaba -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from mangaba_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("mangaba_requires"):
            print(f"Requires:     Mangaba {data['mangaba_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from mangaba_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.mangaba_requires:
        print(f"  Requires: Mangaba {mf.mangaba_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``mangaba dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``mangaba dashboard --status``
    ...
    print(f"{len(pids)} mangaba dashboard process(es) running:")
    for pid in pids:
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from mangaba_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``mangaba -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``mangaba_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``mangaba -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``mangaba`` or only flags → defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from mangaba_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("MANGABA_TUI") == "1" or "--tui" in argv:
        return False
    ...
    if getattr(args, "oneshot", None):
        ...
        from mangaba_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MANGABA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MANGABA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MANGABA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `mangaba --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("MANGABA_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
...
def main():
    ...
    # =========================================================================
    from mangaba_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `mangaba model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.mangaba/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://dheiver2.github.io/mangaba-agent/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from mangaba_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from mangaba_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy mangaba.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Mangaba gateway unit files "
            "(e.g. mangaba.service) left over from older installs. Profile "
            "units (mangaba-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `mangaba proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Mangaba Agent with an interactive wizard. "
        "Run a specific section: mangaba setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'mangaba setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Mangaba.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $MANGABA_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Mangaba")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from mangaba_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Mangaba CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: mangaba-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Mangaba with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set MANGABA_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Mangaba Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.mangaba/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Mangaba profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.mangaba/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Mangaba profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from mangaba_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from mangaba_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.mangaba/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.mangaba/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Mangaba Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `mangaba "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Mangaba setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Mangaba Agent. Use 'mangaba debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    mangaba debug share              Upload debug report and print URL
    mangaba debug share --lines 500  Include more log lines
    mangaba debug share --expire 30  Keep paste for 30 days
    mangaba debug share --local      Print report locally (no upload)
    mangaba debug share --no-redact  Disable upload-time secret redaction
    mangaba debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'mangaba debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Mangaba home directory to a zip file",
        description="Create a zip archive of your entire Mangaba configuration, "
        "skills, sessions, and data (excludes the mangaba-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/mangaba-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.mangaba/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo mangaba uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from mangaba_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Mangaba backup from a zip file",
        description="Extract a previously created Mangaba backup into your "
        "Mangaba home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Mangaba Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from mangaba_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.mangaba/skills/.bundled_manifest) "
            "so future 'mangaba update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from mangaba_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from mangaba_cli.skills_hub import skills_command
    ...
    from mangaba_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/mangaba-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `mangaba plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from mangaba_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``mangaba --help``, ``mangaba version``, ``mangaba logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from mangaba_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from mangaba_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_mangaba_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_mangaba_home()}/memories/\n")
        else:
            from mangaba_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'mangaba tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # mangaba tools list [--platform cli]
    ...
    # mangaba tools disable <name...> [--platform cli]
    ...
    # mangaba tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            _require_tty("tools")
            from mangaba_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `mangaba computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `mangaba tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: mangaba computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: mangaba computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Mangaba as an MCP server",
        description=(
            "Manage MCP server connections and run Mangaba as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'mangaba mcp add' to connect to a new server, or\n"
            "'mangaba mcp serve' to expose Mangaba conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Mangaba as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `mangaba mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from mangaba_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_mangaba_home() / "sessions"
            ...
            # Launch mangaba --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Mangaba",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Mangaba",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.mangaba/ (by default a "
        "single restore-point archive is written to ~/.mangaba/backups/ "
        "before apply; restorable with 'mangaba import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from mangaba_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Mangaba Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'mangaba config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another mangaba.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Mangaba Agent",
        description="Remove Mangaba Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Mangaba Agent as an ACP (Agent Client Protocol) server",
        description="Start Mangaba Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Mangaba ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Mangaba provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.mangaba/node/ "
    ...
    def cmd_acp(args):
        """Launch Mangaba Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Mangaba instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `mangaba update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `mangaba profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Mangaba profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Mangaba Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `mangaba dashboard` cmdlines and SIGTERM them directly — the same
    # path `mangaba update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running mangaba dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Mangaba log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    mangaba logs                    Show last 50 lines of agent.log
    mangaba logs -f                 Follow agent.log in real time
    mangaba logs errors             Show last 50 lines of errors.log
    mangaba logs gateway -n 100     Show last 100 lines of gateway.log
    mangaba logs --level WARNING    Only show WARNING and above
    mangaba logs --session abc123   Filter by session ID
    mangaba logs --component tools  Only show tool-related lines
    mangaba logs --since 1h         Lines from the last hour
    mangaba logs --since 30m -f     Follow, starting from 30 min ago
    mangaba logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``mangaba -c Pokemon Agent Dev`` → ``mangaba -c 'Pokemon Agent Dev'``
    ...
    from mangaba_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'mangaba -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (mangaba hooks list, cron
