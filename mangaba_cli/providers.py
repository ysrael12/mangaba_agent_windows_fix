"""
Single source of truth for provider identity in Mangaba Agent.
...
2. **Mangaba overlays** — transport type, auth patterns, aggregator flags,
...
# -- Mangaba overlay ----------------------------------------------------------
# Mangaba-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class MangabaOverlay:
    """Mangaba-specific provider metadata layered on top of models.dev."""
...
MANGABA_OVERLAYS: Dict[str, MangabaOverlay] = {
    "openrouter": MangabaOverlay(
    ...
    "nous": MangabaOverlay(
        ...
        base_url_override="https://inference-api.dheiver2.com/v1",
    ),
    "openai-codex": MangabaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": MangabaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": MangabaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="MANGABA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": MangabaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": MangabaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": MangabaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": MangabaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": MangabaOverlay(
    ...
    "kimi-for-coding": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": MangabaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": MangabaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": MangabaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": MangabaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": MangabaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": MangabaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": MangabaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": MangabaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": MangabaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": MangabaOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": MangabaOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": MangabaOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": MangabaOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": MangabaOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "mangaba", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Mangaba overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Mangaba overlay
    ...
    overlay = MANGABA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + mangaba extra
    ...
    if overlay is not None:
        # Mangaba-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in MANGABA_OVERLAYS
    if provider == "bedrock":
