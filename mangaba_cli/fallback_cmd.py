"""
mangaba fallback — manage the fallback provider chain.
...
Subcommands:
  mangaba fallback [list]   Show the current fallback chain (default when no subcommand)
  mangaba fallback add      Pick provider + model via the same picker as `mangaba model`,
                           then append the selection to the chain
  mangaba fallback remove   Pick an entry to delete from the chain
  mangaba fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.mangaba/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from mangaba_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `mangaba model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  mangaba fallback add")
        print()
        return
    ...
    print("  Docs: https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `mangaba model`, then append the selection to the chain."""
    from mangaba_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `mangaba model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `mangaba fallback list` to view, or `mangaba fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``mangaba fallback [subcommand]``."""
