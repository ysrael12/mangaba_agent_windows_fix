"""
Mangaba Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.mangaba/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Mangaba PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing mangaba-agent or mangaba PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Mangaba Agent" comment and following line
                if '# Mangaba Agent' in line or '# mangaba-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('mangaba' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing mangaba
                if 'mangaba' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the mangaba wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "mangaba",
        Path("/usr/local/bin/mangaba"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains mangaba_cli reference)
                content = wrapper.read_text()
                if 'mangaba_cli' in content or 'mangaba-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``mangaba gateway run`` processes
    ...
    try:
        from mangaba_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from mangaba_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``MANGABA_HOME`` and ``MANGABA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\mangaba\git\cmd``, ``%LOCALAPPDATA%\mangaba\git\bin``,
#      ``%LOCALAPPDATA%\mangaba\git\usr\bin``, ``%LOCALAPPDATA%\mangaba\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\mangaba\git\`` and Node to
#      ``%LOCALAPPDATA%\mangaba\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``mangaba dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\mangaba\gateway-service\`` and sometimes
...
def _mangaba_path_markers(mangaba_home: Path) -> list[str]:
    """Path-entry substrings that identify Mangaba-owned User-PATH entries."""
    root = str(mangaba_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare mangaba-agent install dir.
    markers = [root + "\\mangaba-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if MANGABA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "mangaba".  We don't want to catch
    # unrelated entries like "cmangaba-foo" or "ephermeral", so we look for
    # backslash-mangaba as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(mangaba_home: Path) -> list[str]:
    """Strip Mangaba-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _mangaba_path_markers(mangaba_home)
            kept: list[str] = []
...
def remove_mangaba_env_vars_windows() -> list[str]:
    """Delete MANGABA_HOME and MANGABA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("MANGABA_HOME", "MANGABA_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MANGABA_HOME directory.

    We shell out to ``mangaba -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MANGABA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                mangaba_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's MANGABA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.mangaba/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.mangaba/ for future reinstall
    """
    project_root = get_project_root()
    mangaba_home = get_mangaba_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie MANGABA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_mangaba_home(mangaba_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Mangaba Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {mangaba_home / 'config.yaml'}")
    print(f"  Secrets: {mangaba_home / '.env'}")
    print(f"  Data:    {mangaba_home / 'cron/'}, {mangaba_home / 'sessions/'}, {mangaba_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their MANGABA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Mangaba data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Mangaba code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in mangaba_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\mangaba\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(mangaba_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Mangaba-owned PATH entries in User environment")

        log_info("Removing MANGABA_HOME / MANGABA_GIT_BASH_PATH User env vars...")
        removed_env = remove_mangaba_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Mangaba-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing mangaba command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.mangaba/, just remove the mangaba-agent subdir
            if mangaba_home in project_root.parents or project_root.parent == mangaba_home:
    ...
    #     under MANGABA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(mangaba_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(mangaba_home)
    ...
    # 5. Optionally remove ~/.mangaba/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile MANGABA_HOME dirs live under
        ...
        try:
            if mangaba_home.exists():
                shutil.rmtree(mangaba_home)
                log_success(f"Removed {mangaba_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {mangaba_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {mangaba_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {mangaba_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/dheiver2/mangaba-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/dheiver2/mangaba-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Mangaba Agent! ⚕")
    print()
