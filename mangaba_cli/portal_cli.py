"""``mangaba portal`` — small CLI surface for Nous Portal users.
...
already in ``mangaba auth`` or ``mangaba tools``. It's a discovery + status
...
DEFAULT_PORTAL_URL = "https://portal.dheiver2.com"
SUBSCRIPTION_URL = "https://portal.dheiver2.com/manage-subscription"
DOCS_URL = "https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   mangaba auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `mangaba model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `mangaba auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `mangaba portal <subcommand>`."""
    ...
    print("Run `mangaba portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `mangaba portal` on the given argparse subparsers object."""
