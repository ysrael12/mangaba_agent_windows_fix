"""
Profile management for multiple isolated Mangaba instances.

Each profile is a fully independent MANGABA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.mangaba/profiles/<name>/`` by default.

The "default" profile is ``~/.mangaba`` itself — backward compatible,
zero migration needed.

Usage::

    mangaba profile create coder          # fresh profile + bundled skills
    mangaba profile create coder --clone  # also copy config, .env, SOUL.md, skills
    mangaba profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    mangaba -p coder chat                 # or via flag
    mangaba profile use coder             # set as sticky default
    mangaba profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See mangaba_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.mangaba``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   mangaba-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `mangaba profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `mangaba
...
# `mangaba skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Mangaba infrastructure directories that only the default profile
         (``~/.mangaba``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_mangaba_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.mangaba) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "mangaba-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "mangaba", "default", "test", "tmp", "root", "sudo",
})

# Mangaba subcommands that cannot be used as profile names/aliases
_MANGABA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the mangaba root, NOT to the current MANGABA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where MANGABA_HOME points outside
    ``~/.mangaba``, profiles live under ``MANGABA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_mangaba_home() / "profiles"


def _get_default_mangaba_home() -> Path:
    """Return the default (pre-profile) MANGABA_HOME path.

    In standard deployments this is ``~/.mangaba``.
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_mangaba_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Mangaba installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MANGABA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_mangaba_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, mangaba subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _MANGABA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a mangaba subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``mangaba profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Mangaba config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_mangaba_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``mangaba update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.mangaba)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.mangaba/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `mangaba update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`mangaba profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `mangaba update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `mangaba profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches MANGABA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``mangaba profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MANGABA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.mangaba).\n"
            "To remove everything, use: mangaba uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set MANGABA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("MANGABA_HOME")
    try:
        os.environ["MANGABA_HOME"] = str(profile_dir)
        from mangaba_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["MANGABA_HOME"] = old_home
        elif "MANGABA_HOME" in os.environ:
            del os.environ["MANGABA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.mangaba/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: mangaba profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from MANGABA_HOME.

    Returns ``"default"`` if MANGABA_HOME is not set or points to ``~/.mangaba``.
    ...
    default_resolved = _get_default_mangaba_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.mangaba itself — its parent is ~/ and its
        # directory name is ".mangaba", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: mangaba profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.mangaba itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.mangaba). "
            "Specify a different name: mangaba profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="mangaba_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"mangaba.{old_name}"
    new_host = f"mangaba.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_mangaba_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MANGABA_HOME path string.

    Called early in the CLI entry point, before any mangaba modules
    are imported, to set the MANGABA_HOME environment variable.
    """
