...
Add, remove, or reorder entries here — both `mangaba setup` and
`mangaba` provider-selection will pick up the change automatically.
"""
...
_MANGABA_USER_AGENT = f"mangaba-cli/{_MANGABA_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `mangaba model`
    flow without maintaining a separate static list.
    """
    from mangaba_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $MANGABA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $MANGABA_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Mangaba refreshes the cache from
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.dheiver2.com").rstrip("/")
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Mangaba that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Mangaba that doesn't ship
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from mangaba_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.dheiver2.com"
...
# this list:  mangaba model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `mangaba model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `mangaba model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Dheiver Santos subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``mangaba model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``mangaba auth add openai-codex`` without
    ``mangaba model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    mangaba-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from mangaba_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — mangaba-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from mangaba_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return mangaba-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; mangaba's picker expects
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.dheiver2.com"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from mangaba_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.dheiver2.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``mangaba model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from mangaba_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from mangaba_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Mangaba' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``mangaba_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Mangaba should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``mangaba auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.mangaba/.env``.
    ...
    """
    try:
        from mangaba_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from mangaba_cli.auth import read_credential_pool
        from mangaba_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Mangaba release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Mangaba release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from mangaba_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from mangaba_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "MangabaAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _MANGABA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Mangaba' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _MANGABA_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_mangaba_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Mangaba will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Mangaba cannot verify the model name."
