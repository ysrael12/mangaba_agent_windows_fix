...
# xAI's public API is OpenAI-compatible for the endpoints Mangaba commonly
# uses. The Responses endpoint is included because Mangaba' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Mangaba-managed OAuth credentials."""

    auth_hint = "mangaba auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`mangaba auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`mangaba auth reset xai-oauth` or re-authenticate with "
                    "`mangaba auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `mangaba auth add xai-oauth --type oauth`."
            )
