"""mangaba hooks — inspect and manage shell-script hooks.

Usage::

    mangaba hooks list
    mangaba hooks test <event> [--for-tool X] [--payload-file F]
    mangaba hooks revoke <command>
    mangaba hooks doctor

Consent records live under ``~/.mangaba/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.mangaba/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``mangaba hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: mangaba hooks {list|test|revoke|doctor}")
        print("Run 'mangaba hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.mangaba/config.yaml.")
        print("See `mangaba hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `mangaba hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `mangaba hooks test` and `mangaba hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Mangaba wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `mangaba hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `mangaba hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `mangaba hooks doctor`.")
