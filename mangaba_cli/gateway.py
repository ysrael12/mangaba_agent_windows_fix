...
# display_mangaba_home is imported lazily at call sites to avoid ImportError
# when mangaba_constants is cached from a pre-update version during `mangaba update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "mangaba-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "mangaba CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``mangaba gateway status`` from falsely counting the ``mangaba`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``mangaba gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "mangaba_cli.main gateway",
        "mangaba_cli.main --profile",
        "mangaba_cli.main -p",
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"MANGABA_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. MANGABA_HOME
        ...
        # MANGABA_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "MANGABA_HOME=" in command and f"MANGABA_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``mangaba update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Mangaba profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Mangaba profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``mangaba update`` users
    # with no running gateway until they re-invoke ``mangaba gateway``
...
def _sync_mangaba_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``MANGABA_HOME``.

    Under ``sudo``, ``MANGABA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_mangaba_home` falls back to ``/root/.mangaba`` — the wrong
    profile. The unit file pins ``MANGABA_HOME`` for the actual gateway
    ...
    unit_home = env.get("MANGABA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("MANGABA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["MANGABA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}mangaba gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}mangaba gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `mangaba gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``mangaba gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from mangaba_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (MANGABA_HOME-scoped).
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``mangaba gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``MANGABA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("MANGABA_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "mangaba-gateway"
SERVICE_DESCRIPTION = "Mangaba Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current MANGABA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (MANGABA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary MANGABA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(mangaba_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when MANGABA_HOME is a named profile.

    For ``~/.mangaba/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        mangaba_home: Optional explicit MANGABA_HOME path. Defaults to the current
            ``get_mangaba_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this MANGABA_HOME.

    Default ``~/.mangaba`` returns ``mangaba-gateway`` (backward compatible).
    Profile ``~/.mangaba/profiles/coder`` returns ``mangaba-gateway-coder``.
    Any other MANGABA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Mangaba installs that predate the
# mangaba-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (mangaba-gateway-*.service) and unrelated third-party
# "mangaba" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("mangaba.service",)
...
def _find_legacy_mangaba_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Mangaba gateway units.

    Detects unit files installed by older Mangaba versions that used a
    different service name (e.g. ``mangaba.service`` before the rename to
    ``mangaba-gateway.service``). When both a legacy unit and the current
    ``mangaba-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``mangaba-gateway-coder.service`` and unrelated third-party
      ``mangaba-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``mangaba.service`` running an unrelated
...
def has_legacy_mangaba_units() -> bool:
    """Return True when any legacy Mangaba gateway unit files exist."""
    return bool(_find_legacy_mangaba_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Mangaba gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_mangaba_units()
    if not legacy:
        return
    print_warning("Legacy Mangaba gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current mangaba-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    mangaba gateway migrate-legacy")


def remove_legacy_mangaba_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Mangaba gateway unit files.

    Iterates over whatever ``_find_legacy_mangaba_units()`` returns — which is
    ...
    legacy = _find_legacy_mangaba_units()
    if not legacy:
        print("No legacy Mangaba gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Mangaba gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: mangaba gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo mangaba gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    mangaba gateway uninstall")
    print_info("    sudo mangaba gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Mangaba can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo mangaba gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo mangaba gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo mangaba gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Mangaba often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.mangaba`` → ``ai.mangaba.gateway.plist`` (backward compatible).
    Profile ``~/.mangaba/profiles/coder`` → ``ai.mangaba.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.mangaba.gateway-{suffix}" if suffix else "ai.mangaba.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.mangaba/mangaba-agent  -> /home/alice/.mangaba/mangaba-agent
      /opt/mangaba                 -> /opt/mangaba  (kept as-is)
...
def _mangaba_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MANGABA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_mangaba_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.mangaba                    → /home/alice/.mangaba
      /root/.mangaba/profiles/coder     → /home/alice/.mangaba/profiles/coder
      /opt/custom-mangaba               → /opt/custom-mangaba  (kept as-is)
    """
    current_mangaba = get_mangaba_home().resolve()
    current_default = (Path.home() / ".mangaba").resolve()
    target_default = Path(target_home_dir) / ".mangaba"

    # Default ~/.mangaba → remap to target user's default
    if current_mangaba == current_default:
        return str(target_default)

    # Profile or subdir of ~/.mangaba → preserve the relative structure
    try:
        relative = current_mangaba.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.mangaba) — keep as-is
        return str(current_mangaba)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    mangaba_node = mangaba_home / "node" / "bin"
    if _is_dir(mangaba_node):
        candidates.append(str(mangaba_node))
    mangaba_nm = mangaba_home / "node_modules" / ".bin"
    if _is_dir(mangaba_nm):
        candidates.append(str(mangaba_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        mangaba_home = _mangaba_home_for_target_user(home_dir)
        profile_arg = _profile_arg(mangaba_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m mangaba_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="MANGABA_HOME={mangaba_home}"
Restart=always
RestartSec=5
...
"""
    ...
    mangaba_home = str(get_mangaba_home().resolve())
    profile_arg = _profile_arg(mangaba_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only MANGABA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp MANGABA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../mangaba_test`` path into
    # ``Environment="MANGABA_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Mangaba install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo mangaba gateway uninstall --system")
    print_info("         mangaba gateway install")
    print_info("         mangaba gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (mangaba.service from pre-rename installs)
    # before installing the new mangaba-gateway.service. If both remain, they
    ...
    if has_legacy_mangaba_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_mangaba_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}mangaba gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}mangaba gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}mangaba gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_mangaba_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `mangaba gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `mangaba gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}mangaba gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}mangaba gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}mangaba gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}mangaba gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.mangaba.gateway-{suffix}" if suffix else "ai.mangaba.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>mangaba_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>MANGABA_HOME</key>
        <string>{mangaba_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Mangaba install")
    return True
...
def launchd_stop():
    ...
    # `mangaba gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Mangaba install")
    else:
        print("⚠ Service definition is stale relative to the current Mangaba install")
        print("  Run: mangaba gateway start")
    ...
    if deep:
        log_file = get_mangaba_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("MANGABA_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Mangaba gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'mangaba' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/mangaba/docker/entrypoint.sh before the Mangaba command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$MANGABA_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set MANGABA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `mangaba gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set MANGABA_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `mangaba gateway restart` which already
    ...
    # `mangaba gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Mangaba Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # MANGABA_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Mangaba will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @mangaba:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. mangaba) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Mangaba delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Mangaba agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Mangaba will use (e.g., mangaba@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Mangaba connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: mangaba pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Mangaba will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``mangaba setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run mangaba under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.mangaba/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_mangaba_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'mangaba pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: mangaba pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'mangaba gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from mangaba_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from mangaba_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from mangaba_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Mangaba will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Mangaba will store the returned account_id/token in ~/.mangaba/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `mangaba gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_mangaba_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `mangaba pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `mangaba pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"MangabaAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``mangaba_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from mangaba_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.mangaba/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.mangaba/.env: {', '.join(required)}")
    else:
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: mangaba gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: mangaba gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: mangaba gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo mangaba gateway install --system")
                    print_info("  Or run in foreground:  mangaba gateway run")
            elif is_wsl():
        ...
        print()
        print_info("No platforms configured. Run 'mangaba gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``mangaba gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: mangaba gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: mangaba gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s mangaba 'mangaba gateway run'")
                print()
            ...
            print()
            print("  mangaba gateway run                              # direct foreground")
            print("  tmux new -s mangaba 'mangaba gateway run'         # persistent via tmux")
            print("  nohup mangaba gateway run > ~/.mangaba/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: mangaba gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: mangaba gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: mangaba gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: mangaba gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: mangaba gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  mangaba gateway install")
                else:
                    print("To install as a service:")
                    print("  mangaba gateway install")
                    print("  sudo mangaba gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  mangaba gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup mangaba gateway run > ~/.mangaba/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s mangaba 'mangaba gateway run'         # persistent via tmux")
                    print("  nohup mangaba gateway run > ~/.mangaba/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  mangaba gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  mangaba gateway install  # Install as user service")
                    print("  sudo mangaba gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Mangaba gateway unit files from
        # pre-rename installs (e.g. mangaba.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_mangaba_units(interactive=not yes, dry_run=dry_run)
