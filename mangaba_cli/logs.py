"""``mangaba logs`` — view and filter Mangaba log files.
...
Usage examples::
    ...
    mangaba logs                    # last 50 lines of agent.log
    mangaba logs -f                 # follow agent.log in real time
    mangaba logs errors             # last 50 lines of errors.log
    mangaba logs gateway -n 100    # last 100 lines of gateway.log
    mangaba logs --level WARNING    # only WARNING+ lines
    mangaba logs --session abc123   # filter by session ID substring
    mangaba logs --component tools  # only tool-related lines
    mangaba logs --since 1h         # lines from the last hour
    mangaba logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_mangaba_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Mangaba runs — try 'mangaba chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_mangaba_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_mangaba_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_mangaba_home()}/logs/")
        return

    print(f"Log files in {display_mangaba_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'mangaba chat' to generate logs)")
