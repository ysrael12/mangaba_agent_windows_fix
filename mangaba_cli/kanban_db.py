...
``<root>`` is the **shared Mangaba root** (the parent of any active
...
``mangaba -p <profile>`` joins the same board as the dispatcher that
...
the profile's Mangaba home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``MANGABA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``MANGABA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``mangaba kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.mangaba``. In Docker / custom
deployments where ``MANGABA_HOME`` points outside ``~/.mangaba`` (e.g.
``/opt/mangaba``), ``<root>`` is ``MANGABA_HOME``. Legacy env-var
overrides still work:

* ``MANGABA_KANBAN_DB`` — pin the database file path directly.
* ``MANGABA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``MANGABA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``MANGABA_KANBAN_DB``,
``MANGABA_KANBAN_WORKSPACES_ROOT``, and ``MANGABA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/mangaba-kanban-v1-spec.pdf`` for the full
...
# ``MANGABA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``MANGABA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("MANGABA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``mangaba-agent``
...
def kanban_home() -> Path:
    """Return the shared Mangaba root that anchors the kanban board.

    Resolution order:

    1. ``MANGABA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_mangaba_root()``, which already returns ``<root>``
       when ``MANGABA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``MANGABA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``MANGABA_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``mangaba kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``MANGABA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``mangaba kanban boards
    ...
    """
    env = os.environ.get("MANGABA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``mangaba kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``MANGABA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("MANGABA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``MANGABA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("MANGABA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``mangaba kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``MANGABA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``MANGABA_SESSION_ID``. NULL
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Mangaba treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``MANGABA_KANBAN_DB`` env → ``MANGABA_KANBAN_BOARD`` env →
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``mangaba kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``MANGABA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from mangaba_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``mangaba --skills ...`` alongside the built-in
    ...
    # `mangaba --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``mangaba kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `mangaba kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``mangaba kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``mangaba kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``mangaba kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``mangaba kanban complete <id>``) works without requiring
...
#      via ``mangaba kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Mangaba
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``mangaba kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Mangaba profile.
    ...
    if not rows:
        return False
    try:
        from mangaba_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Mangaba profile.
        # `_default_spawn` invokes ``mangaba -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Mangaba
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `mangaba kanban tail` — without
...
def _module_mangaba_argv() -> list[str]:
    """Return the interpreter-bound Mangaba CLI invocation."""
...
def _absolute_mangaba_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Mangaba shim."""
...
def _mangaba_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Mangaba executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_mangaba_argv()
    return [_absolute_mangaba_path(path)]


def _resolve_mangaba_argv() -> list[str]:
    """Resolve the ``mangaba`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$MANGABA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("mangaba")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\mangaba.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m mangaba_cli.main`` — fallback for setups where
       Mangaba is launched from a venv and the ``mangaba`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_mangaba_bin`` for the same reason. Kept
    local (not imported from gateway) because ``mangaba_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("MANGABA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _mangaba_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _mangaba_path_argv(resolved_env_bin)
        return _module_mangaba_argv()

    mangaba_bin = _safe_which_no_cwd("mangaba") if _IS_WINDOWS else shutil.which("mangaba")
    if mangaba_bin:
        return _mangaba_path_argv(mangaba_bin)
    return _module_mangaba_argv()


def _kanban_worker_skill_available(mangaba_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``mangaba -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset MANGABA_HOME means the worker falls back to the default root
    # home (``~/.mangaba``), which ships the bundled skill.
    base = _Path(mangaba_home) if mangaba_home else (_Path.home() / ".mangaba")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``mangaba -p <profile> chat -q ...`` subprocess.
    ...
    ``MANGABA_KANBAN_DB`` / ``MANGABA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject MANGABA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `mangaba -p <name>` the
    # _apply_profile_override() runs *before* mangaba_constants is imported.
    # If MANGABA_HOME is absent from the child's env, get_mangaba_home() falls
    # back to Path.home() / ".mangaba" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from mangaba_cli.profiles import resolve_profile_env
    try:
        env["MANGABA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via MANGABA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # MANGABA_HOME never had profiles created.
        pass
    if task.tenant:
        env["MANGABA_TENANT"] = task.tenant
    env["MANGABA_KANBAN_TASK"] = task.id
    env["MANGABA_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["MANGABA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["MANGABA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["MANGABA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`mangaba -p <name>`
    # rewrites MANGABA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_mangaba_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["MANGABA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["MANGABA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["MANGABA_KANBAN_BOARD"] = resolved_board
    # MANGABA_PROFILE is the author the kanban_comment tool defaults to.
    # `mangaba -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["MANGABA_PROFILE"] = profile_arg

    cmd = [
        *_resolve_mangaba_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped MANGABA_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("MANGABA_HOME")):
    ...
    # `mangaba kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`mangaba` executable not found on PATH. "
            "Install Mangaba Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``mangaba kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled MANGABA_PROFILE values like "mangaba-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Mangaba root exists

    Reads profile paths directly so this module has no import dependency on
    ``mangaba_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from mangaba_agent.mangaba_constants import get_default_mangaba_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``mangaba kanban assignees`` for the terminal.
