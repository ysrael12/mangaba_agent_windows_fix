"""Shell completion script generation for mangaba CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_mangaba_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Mangaba Agent bash completion
# Add to ~/.bashrc:
#   eval "$(mangaba completion bash)"

_mangaba_profiles() {{
    local profiles_dir="$HOME/.mangaba/profiles"
...
_mangaba_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_mangaba_profiles)" -- "$cur"))
        return
    fi
...
complete -F _mangaba_completion mangaba
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef mangaba
# Mangaba Agent zsh completion
# Add to ~/.zshrc:
...
_mangaba_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.mangaba/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.mangaba/profiles 2>/dev/null)}}")
    fi
...
_mangaba() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_mangaba_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'mangaba command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Mangaba Agent fish completion",
        "# Add to your config:",
        "#   mangaba completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __mangaba_profiles",
        "    echo default",
        "    if test -d $HOME/.mangaba/profiles",
        "        ls $HOME/.mangaba/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c mangaba -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c mangaba -f -s p -l profile"
        " -d 'Profile name' -xa '(__mangaba_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__mangaba_profiles)' -d 'Profile name'"
                )
