"""`mangaba checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.mangaba/checkpoints/``.  Actions:

    mangaba checkpoints               # same as `status`
    mangaba checkpoints status        # total size, project count, breakdown
    mangaba checkpoints list          # per-project checkpoint counts + workdir
    mangaba checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    mangaba checkpoints clear [-f]    # nuke the entire base (asks first)
    mangaba checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    mangaba checkpoints
    mangaba checkpoints prune --retention-days 3 --max-size-mb 200
    mangaba checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: mangaba checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``mangaba checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `mangaba checkpoints` → status
