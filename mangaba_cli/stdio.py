...
on Japanese locales, etc.).  Mangaba's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``mangaba_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Mangaba is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MANGABA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MANGABA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Mangaba-managed Git install directories so
    ...
    # already-running shells don't see the change — which means mangaba
    ...
    # install without Mangaba-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Mangaba's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Mangaba's
    ``mangaba config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Mangaba (User env var in Windows Settings, or
    export in a PowerShell profile) and Mangaba picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Mangaba-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\mangaba\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``mangaba`` from right after install) retain their old PATH.

    Any subprocess Mangaba spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Mangaba-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "mangaba", "git", "cmd"),
        os.path.join(local_appdata, "mangaba", "git", "bin"),
        os.path.join(local_appdata, "mangaba", "git", "usr", "bin"),
        # Mangaba venv Scripts directory — host of the mangaba.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes mangaba, but harmless to include.
        os.path.join(local_appdata, "mangaba", "mangaba-agent", "venv", "Scripts"),
