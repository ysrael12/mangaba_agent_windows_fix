"""Slash command definitions and autocomplete for the Mangaba CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Mangaba config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Mangaba works on across turns until achieved", "Session",
    ...
    CommandDef("set", "Set a config value or secret (routes tokens to .env)",
               "Configuration", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               args_hint="<chave> <valor>"),
    CommandDef("soul", "View or edit the agent identity (SOUL.md)",
               "Configuration", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               args_hint="[show|set <texto>]"),
    CommandDef("rules", "View or edit work rules (MANGABA.md)",
               "Configuration", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               args_hint="[show|set <texto>]"),
    CommandDef("modelo", "Show the active model, its tier (capaz/fraco) and what it activates",
               "Configuration", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("tier",)),
    ...
    CommandDef("busy", "Control what Enter does while Mangaba is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.mangaba/skills/ for newly installed or removed skills",
    ...
    CommandDef("mcp", "Manage MCP servers: /mcp [list|add <nome> <url>|remove <nome>]",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               args_hint="[list|add <nome> <url>|remove <nome>]"),
    CommandDef("security", "Scan this install for leaked secrets and risky config",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("scan",)),
    CommandDef("instinct", "Learned 'when X → do Y' rules auto-injected into the prompt",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("instintos", "instinto"),
               args_hint="[list|add <gatilho> :: <ação>|extract|forget <id>|promote]"),
    CommandDef("followup", "Schedule a proactive follow-up message in this chat (heartbeat)",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("followups", "lembrete"),
               args_hint="[list|add <quando> :: <mensagem>|cancel <id>]"),
    CommandDef("tarefa", "Decompose a complex request into a step-by-step plan, then execute",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("plano", "task", "resolver"),
               args_hint="<pedido complexo>"),
    CommandDef("fleet", "Manage all agents (profiles): status and restart from the channel",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("frota",),
               args_hint="[list|status|restart <nome>]"),
    CommandDef("agent", "Create, delete or list agents (profiles) from the channel",
               "Tools & Skills", cli_only=True,
               gateway_config_gate="gateway.expose_admin_commands",
               aliases=("agente",),
               args_hint="[list|create <nome> [descrição]|delete <nome>]"),
    ...
    CommandDef("update", "Update Mangaba Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`mangaba_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/mangaba`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Mangaba
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``mangaba skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``mangaba skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``mangaba skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/mangaba <verb>`` subcommand.
    ...
    can still reach them via ``/mangaba <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/mangaba`` is always reserved as the first entry so the
    legacy ``/mangaba <subcommand>`` form keeps working for anything that
    ...
    # Reserve /mangaba as the catch-all top-level command.
    entries.append(("mangaba", "Talk to Mangaba or run a subcommand", "[subcommand] [args]"))
    seen.add("mangaba")
...
def slack_app_manifest(request_url: str = "https://mangaba-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /mangaba handler.

    Maps both canonical names and aliases so /mangaba bg do stuff works
    the same as /mangaba background do stuff.

    Plugin-registered slash commands are included so ``/mangaba <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from mangaba_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
