"""
Dump command for mangaba CLI.

Outputs a compact, plain-text summary of the user's Mangaba setup
...
def _redact(value: str) -> str:
    ...
    ``mangaba dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from mangaba_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(mangaba_home: Path) -> int:
...
def _cron_summary(mangaba_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = mangaba_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"mangaba_home:      {display_mangaba_home()}")
    ...
    toolsets = config.get("toolsets", ["mangaba-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(mangaba_home)}")
    lines.append(f"  skills:             {_count_skills(mangaba_home)}")
