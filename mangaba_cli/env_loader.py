"""Helpers for loading Mangaba .env files consistently across entrypoints."""
...
# load_mangaba_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_mangaba_dotenv().
# Used by setup / `mangaba model` flows to label detected credentials so
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_mangaba_dotenv()`` call.  Returns
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `mangaba setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``mangaba_cli.config._sanitize_env_lines`` which
    already knows all valid Mangaba env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from mangaba_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_mangaba_dotenv(
    *,
    mangaba_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Mangaba environment files with user config taking precedence.

    Behavior:
    - `~/.mangaba/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(mangaba_home or os.getenv("MANGABA_HOME", Path.home() / ".mangaba"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Mangaba reads
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `mangaba model`
