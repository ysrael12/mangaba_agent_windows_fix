"""Migrate Mangaba' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Mangaba' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Mangaba mcp_servers.<n>.command/args/env  → codex stdio transport
  Mangaba mcp_servers.<n>.url/headers       → codex streamable_http transport
  Mangaba mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Mangaba mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Mangaba-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Mangaba' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Mangaba config.")
...
# Mangaba keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Mangaba' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, mangaba_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Mangaba MCP server config to the codex inline-table dict
    ...
    if not isinstance(mangaba_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(mangaba_cfg.get("command"))
    has_url = bool(mangaba_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (MANGABA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Mangaba)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Mangaba' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Mangaba' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Mangaba
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="mangaba-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a MANGABA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed mangaba-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.mangaba`` symlink that happens to live under ``/private/var/folders``
...
def _build_mangaba_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Mangaba' own
    ...
    so a mangaba installed under /opt/, /usr/local/, or a venv all work.
    MANGABA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # MANGABA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_mangaba_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever MANGABA_HOME its launcher sets
    ...
    # — that would override the launcher's MANGABA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("MANGABA_HOME", tmp_path) would
    ...
    mangaba_home = os.environ.get("MANGABA_HOME") or ""
    if mangaba_home and _looks_like_test_tempdir(mangaba_home):
        mangaba_home = ""
    if mangaba_home:
        env["MANGABA_HOME"] = mangaba_home
    # PYTHONPATH passes through so a worktree-launched mangaba finds the
    ...
    env["MANGABA_QUIET"] = "1"
    env["MANGABA_REDACT_SECRETS"] = env.get("MANGABA_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_mangaba_tools: bool = True,
) -> MigrationReport:
    """Translate Mangaba mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        mangaba_config: full ~/.mangaba/config.yaml dict
        ...
        expose_mangaba_tools: when True (default), register Mangaba' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Mangaba for tools
    ...
    mangaba_servers = (mangaba_config or {}).get("mcp_servers") or {}
    if not isinstance(mangaba_servers, dict):
        report.errors.append(
            "mcp_servers in Mangaba config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in mangaba_servers.items():
    ...
    # Inject Mangaba' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Mangaba for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/mangaba_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_mangaba_tools:
        translated["mangaba-tools"] = _build_mangaba_tools_mcp_entry()
        if "mangaba-tools" not in report.migrated:
            report.migrated.append("mangaba-tools")
