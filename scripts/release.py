...
VERSION_FILE = REPO_ROOT / "mangaba_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+mangaba-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@dheiver2.com": "shannonsands",
    ...
    "emozilla@dheiver2.com": "emozilla",
    ...
    "mangaba@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@mangaba-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MANGABA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MANGABA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: mangaba-eval community link)
    ...
    "mangabaagent26@gmail.com": "mangabaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+mangabaagent26@users.noreply.github.com": "mangabaagent26",
    ...
    "clement@dheiver2.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@mangaba": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.mangaba@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"mangaba-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "mangaba@dheiver2.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/dheiver2/mangaba-agent",
    ...
    lines.append(f"# Mangaba Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Mangaba Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Mangaba Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Mangaba Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Mangaba Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Mangaba Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Mangaba Agent v{new_version} ({calver_date})' "
