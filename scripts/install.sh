...
# Python-driven tool session (e.g. Mangaba terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:dheiver2/mangaba-agent.git"
REPO_URL_HTTPS="https://github.com/dheiver2/mangaba-agent.git"
MANGABA_HOME="${MANGABA_HOME:-$HOME/.mangaba}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${MANGABA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MANGABA_INSTALL_DIR"
...
#   code at /usr/local/lib/mangaba-agent, command at /usr/local/bin/mangaba,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.mangaba/mangaba-agent"
            echo "                   default (root, Linux): /usr/local/lib/mangaba-agent"
            echo "  --mangaba-home PATH  Data directory (default: ~/.mangaba, or \$MANGABA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Mangaba installs the code under"
            echo "  /usr/local/lib/mangaba-agent and links the command into"
            echo "  /usr/local/bin/mangaba (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MANGABA_HOME"
            echo "  (default /root/.mangaba).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MANGABA_HOME/mangaba-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs mangaba setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Mangaba Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Dheiver Santos.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `mangaba` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MANGABA_HOME/mangaba-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MANGABA_HOME/mangaba-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/mangaba-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MANGABA_HOME/mangaba-agent — then preserve it)
#
# Always no-op when the user set --dir or $MANGABA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MANGABA_HOME.
    if is_termux; then
        INSTALL_DIR="$MANGABA_HOME/mangaba-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MANGABA_HOME/mangaba-agent/.git" ]; then
            INSTALL_DIR="$MANGABA_HOME/mangaba-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/mangaba-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/mangaba-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/mangaba"
        log_info "  Data:    $MANGABA_HOME (unchanged)"
        return 0
    fi
...
get_mangaba_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/mangaba" ]; then
        echo "$link_dir/mangaba"
    else
        echo "mangaba"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/dheiver2/mangaba-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/mangaba-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/mangaba-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$MANGABA_HOME/node/bin/node" ]; then
        export PATH="$MANGABA_HOME/node/bin:$PATH"
        local found_ver=$("$MANGABA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Mangaba-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.mangaba/node/..."
    ...
    # Place into ~/.mangaba/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$MANGABA_HOME/node"
    mkdir -p "$MANGABA_HOME"
    mv "$extracted_dir" "$MANGABA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$MANGABA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$MANGABA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$MANGABA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$MANGABA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$MANGABA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.mangaba/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Mangaba install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Mangaba Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="mangaba-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Mangaba behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"mangaba-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up mangaba command..."

    if [ "$USE_VENV" = true ]; then
        MANGABA_BIN="$INSTALL_DIR/venv/bin/mangaba"
    else
        MANGABA_BIN="$(which mangaba 2>/dev/null || echo "")"
        if [ -z "$MANGABA_BIN" ]; then
            log_warn "mangaba not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$MANGABA_BIN" ]; then
        log_warn "mangaba entry point not found at $MANGABA_BIN"
    ...
    # Create a user-facing shim for the mangaba command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $MANGABA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$MANGABA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/mangaba"
    cat > "$command_link_dir/mangaba" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MANGABA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/mangaba"
    log_success "Installed mangaba launcher → $command_link_display_dir/mangaba"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "mangaba command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v mangaba' \
                >/dev/null 2>&1; then
        ...
        log_info "mangaba not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Mangaba Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Mangaba Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Mangaba Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so mangaba works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.mangaba directory structure (config at top level, code in subdir)
    mkdir -p "$MANGABA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.mangaba/.env (top level, easy to find)
    if [ ! -f "$MANGABA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MANGABA_HOME/.env"
            log_success "Created ~/.mangaba/.env from template"
        else
            touch "$MANGABA_HOME/.env"
            log_success "Created ~/.mangaba/.env"
        fi
    else
        log_info "~/.mangaba/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$MANGABA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MANGABA_HOME/config.yaml"
            log_success "Created ~/.mangaba/config.yaml from template"
        fi
    else
        log_info "~/.mangaba/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$MANGABA_HOME/SOUL.md" ]; then
        cat > "$MANGABA_HOME/SOUL.md" << 'SOUL_EOF'
# Mangaba Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.mangaba/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.mangaba/"

    # Seed bundled skills into ~/.mangaba/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.mangaba/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.mangaba/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MANGABA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$MANGABA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.mangaba/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (mangaba --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'mangaba setup' after install."
        return 0
    fi
    ...
    # Run mangaba setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m mangaba_cli.main setup < /dev/tty
    else
        python -m mangaba_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$MANGABA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Mangaba to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MANGABA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'mangaba whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MANGABA_CMD="$(get_mangaba_command_path)"
                $MANGABA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'mangaba whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'mangaba gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MANGABA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MANGABA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: mangaba gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: mangaba gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $MANGABA_CMD gateway > "$MANGABA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.mangaba/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: mangaba gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: mangaba gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MANGABA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MANGABA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MANGABA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}mangaba${NC}              Start chatting"
    echo -e "   ${GREEN}mangaba setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}mangaba config${NC}       View/edit configuration"
    echo -e "   ${GREEN}mangaba config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}mangaba gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}mangaba update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'mangaba' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'mangaba' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'mangaba' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$MANGABA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$MANGABA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$MANGABA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$MANGABA_HOME/node" --silent --ignore-scripts \
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Mangaba for pip install"
    ...
    MANGABA_CMD="$(command -v mangaba 2>/dev/null || echo "")"
    if [ -n "$MANGABA_CMD" ]; then
        log_info "Running mangaba setup..."
        "$MANGABA_CMD" setup
    else
        log_warn "mangaba command not found on PATH"
        log_info "Try: python -m mangaba_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$MANGABA_HOME/.install_method"
}
