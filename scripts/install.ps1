# ============================================================================
# Mangaba Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$MangabaHome = "$env:LOCALAPPDATA\mangaba",
    [string]$InstallDir = "$env:LOCALAPPDATA\mangaba\mangaba-agent",
...
$RepoUrlSsh = "git@github.com:dheiver2/mangaba-agent.git"
$RepoUrlHttps = "https://github.com/dheiver2/mangaba-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Mangaba Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Dheiver Santos.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $MangabaHome "node"
...
function Install-Git {
    ...
    which Mangaba uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\mangaba\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Mangaba needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Mangaba copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\mangaba\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MANGABA_GIT_BASH_PATH`` (User scope) so Mangaba can find it in a fresh
    ...
    # Download PortableGit into $MangabaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MangabaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Mangaba features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Mangaba needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MANGABA_GIT_BASH_PATH`` (User env scope) so Mangaba can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MangabaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MangabaHome\git\usr\bin\bash.exe
    $candidates += "$MangabaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MangabaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MANGABA_GIT_BASH_PATH", $candidate, "User")
            $env:MANGABA_GIT_BASH_PATH = $candidate
            Write-Info "Set MANGABA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Mangaba may not find Git Bash."
    Write-Info "If needed, set MANGABA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$MangabaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$MangabaHome\node;$env:Path"
        Write-Success "Node.js $version found (Mangaba-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $MangabaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MangabaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\mangaba-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$MangabaHome\node") { Remove-Item -Recurse -Force "$MangabaHome\node" }
                Move-Item $extractedDir.FullName "$MangabaHome\node"
                ...
                $version = & "$MangabaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MangabaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running mangaba processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/dheiver2/mangaba-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/dheiver2/mangaba-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/dheiver2/mangaba-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\mangaba-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\mangaba-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `mangaba.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install mangaba-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and mangaba.exe broken with
    ...
    # here, not 30 seconds later when the user runs `mangaba`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `mangaba dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `mangaba dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `mangaba dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up mangaba command..."
    
    if ($NoVenv) {
        $mangabaBin = "$InstallDir"
    } else {
        $mangabaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so mangaba is globally available
    # On Windows, the mangaba.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$mangabaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$mangabaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $mangabaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MANGABA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\mangaba instead
    # of the Unix default ~/.mangaba
    $currentMangabaHome = [Environment]::GetEnvironmentVariable("MANGABA_HOME", "User")
    if (-not $currentMangabaHome -or $currentMangabaHome -ne $MangabaHome) {
        [Environment]::SetEnvironmentVariable("MANGABA_HOME", $MangabaHome, "User")
        Write-Success "Set MANGABA_HOME=$MangabaHome"
    }
    $env:MANGABA_HOME = $MangabaHome
    
    # Update current session
    $env:Path = "$mangabaBin;$env:Path"
    
    Write-Success "mangaba command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.mangaba directory structure
    New-Item -ItemType Directory -Force -Path "$MangabaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MangabaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MangabaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.mangaba/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.mangaba/.env"
        }
    } else {
        Write-Info "~/.mangaba/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$MangabaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.mangaba/config.yaml from template"
        }
    } else {
        Write-Info "~/.mangaba/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Mangaba's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.mangaba/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.mangaba/"
    
    # Seed bundled skills into ~/.mangaba/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.mangaba/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.mangaba/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MangabaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.mangaba/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'mangaba setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\mangaba-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\mangaba-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\mangaba-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.mangaba/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'mangaba setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m mangaba_cli.main setup
    } else {
        python -m mangaba_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $mangabaCmd = "$InstallDir\venv\Scripts\mangaba.exe"
    if (-not (Test-Path $mangabaCmd)) {
        $mangabaCmd = "mangaba"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MangabaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'mangaba whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: mangaba gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MangabaHome\logs\gateway.log"
            Start-Process -FilePath $mangabaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MangabaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: mangaba gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: mangaba gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MangabaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MangabaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MangabaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MangabaHome\mangaba-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   mangaba              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   mangaba setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   mangaba config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   mangaba config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   mangaba gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   mangaba update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Mangaba repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Mangaba to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/dheiver2/mangaba-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
