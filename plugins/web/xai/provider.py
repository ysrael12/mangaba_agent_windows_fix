...
``{title, url, description, position}`` rows every other Mangaba web
provider produces.
...
prefers Mangaba-managed xAI Grok OAuth (via ``mangaba auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.mangaba/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``mangaba tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `mangaba auth` to sign in with "
