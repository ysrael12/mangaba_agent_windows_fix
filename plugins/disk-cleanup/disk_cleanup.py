"""disk_cleanup — ephemeral file cleanup for Mangaba Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under MANGABA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MANGABA_HOME and /tmp/mangaba-*
Never touches: ~/.mangaba/logs/ or any system directory.
"""
...
try:
    ...
    def get_mangaba_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MANGABA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".mangaba").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MANGABA_HOME/logs/``."""
    return get_mangaba_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MANGABA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MANGABA_HOME or ``/tmp/mangaba-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    mangaba_home = get_mangaba_home()
    try:
        path.resolve().relative_to(mangaba_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/mangaba-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("mangaba-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MANGABA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MANGABA_HOME (but leave MANGABA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "mangaba-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(mangaba_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == mangaba_home:
                continue
            try:
                rel_parts = dirpath.relative_to(mangaba_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    mangaba_home = get_mangaba_home()
    try:
        rel = path.resolve().relative_to(mangaba_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "mangaba-agent",
        }:
            return None
        ...
        # Path isn't under MANGABA_HOME (e.g. /tmp/mangaba-*) — fall through.
        pass
