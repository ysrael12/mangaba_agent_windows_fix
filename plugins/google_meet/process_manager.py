...
session-scoped state file under ``$MANGABA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $MANGABA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_mangaba_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this mangaba install, leave it first —
    ...
    env = os.environ.copy()
    env["MANGABA_MEET_URL"] = url
    env["MANGABA_MEET_OUT_DIR"] = str(out)
    env["MANGABA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["MANGABA_MEET_HEADED"] = "1"
    if auth_state:
        env["MANGABA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["MANGABA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if MANGABA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["MANGABA_MEET_MODE"] = mode
    if realtime_model:
        env["MANGABA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["MANGABA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["MANGABA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["MANGABA_MEET_REALTIME_KEY"] = realtime_api_key
