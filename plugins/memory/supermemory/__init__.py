...
_DEFAULT_CONTAINER_TAG = "mangaba"
...
def _load_supermemory_config(mangaba_home: str) -> dict:
    config = _default_config()
    config_path = Path(mangaba_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, mangaba_home: str) -> None:
    config_path = Path(mangaba_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `mangaba memory setup`.
        # All other options are documented for $MANGABA_HOME/supermemory.json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._mangaba_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "mangaba", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "mangaba_memory", "target": target, "type": "explicit_memory"},
