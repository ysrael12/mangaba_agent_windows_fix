...
- Per-user tokens (keyed by sender email):
    ``${MANGABA_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${MANGABA_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${MANGABA_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${MANGABA_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${MANGABA_HOME}/google_chat_user_client_secret.json``
"""
...
def _mangaba_home() -> Path:
    """Resolve MANGABA_HOME at call time (NOT module import).

    Tests and ``MANGABA_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.mangaba/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _mangaba_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _mangaba_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _mangaba_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _mangaba_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _mangaba_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install mangaba-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'mangaba-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into MANGABA_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_mangaba_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_mangaba_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Mangaba (native attachment delivery)"
    )
