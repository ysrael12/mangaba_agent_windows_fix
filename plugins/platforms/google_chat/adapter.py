...
# ``model_tools`` import time, so ``mangaba status``, ``mangaba chat``, etc.
...
# ``mangaba_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Mangaba is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a mangaba MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MANGABA_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _mangaba_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Mangaba is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MANGABA_HOME", str(_Path.home() / ".mangaba"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MANGABA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a mangaba MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Mangaba is thinking…' marker message.
        ...
        "Mangaba is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Mangaba is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Mangaba is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Mangaba is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Mangaba is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``mangaba setup``.

    The setup wizard at ``mangaba_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.mangaba/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. mangaba-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.mangaba/.env")
    print_info("Restart the gateway: mangaba gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``mangaba cron`` running as a
    separate process from ``mangaba gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Mangaba plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'mangaba-agent[google_chat]'",
        ...
        # the "Mangaba is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Mangaba is thinking…' marker message "
