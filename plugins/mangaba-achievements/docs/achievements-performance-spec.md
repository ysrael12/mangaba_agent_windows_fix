# Mangaba Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: mangaba-achievements plugin
...
### `GET /api/plugins/mangaba-achievements/achievements`
...
### `POST /api/plugins/mangaba-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/mangaba-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/mangaba-achievements/overview`
...
- Persist plugin data under:
  - `~/.mangaba/plugins/mangaba-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.mangaba/plugins/mangaba-achievements/scan_checkpoint.json`
...
- `~/.mangaba/plugins/mangaba-achievements/`
