...
# 5. Symlinks the 'mangaba' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Mangaba Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/mangaba-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink mangaba into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up mangaba command..."

MANGABA_BIN="$SCRIPT_DIR/venv/bin/mangaba"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MANGABA_BIN" "$COMMAND_LINK_DIR/mangaba"
echo -e "${GREEN}✓${NC} Symlinked mangaba → $COMMAND_LINK_DISPLAY_DIR/mangaba"
...
# ============================================================================
# Seed bundled skills into ~/.mangaba/skills/
# ============================================================================

MANGABA_SKILLS_DIR="${MANGABA_HOME:-$HOME/.mangaba}/skills"
mkdir -p "$MANGABA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.mangaba/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MANGABA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  mangaba status        # Check configuration"
if is_termux; then
    echo "  mangaba gateway       # Run gateway in foreground"
else
    echo "  mangaba gateway install # Install gateway service (messaging + cron)"
fi
echo "  mangaba cron list     # View scheduled jobs"
echo "  mangaba doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m mangaba_cli.main setup
fi
