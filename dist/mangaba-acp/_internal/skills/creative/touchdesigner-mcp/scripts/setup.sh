...
MANGABA_HOME_DIR="${MANGABA_HOME:-$HOME/.mangaba}"
MANGABA_CFG="${MANGABA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Mangaba config has twozero_td MCP entry ──
if [[ ! -f "$MANGABA_CFG" ]]; then
    echo -e " ${FAIL} Mangaba config not found at ${MANGABA_CFG}"
    manual_steps+=("Create ${MANGABA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MANGABA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Mangaba config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Mangaba config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$MANGABA_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MANGABA_CFG} manually"); }
    manual_steps+=("Restart Mangaba session to pick up config change")
fi
