...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MANGABA_HOME", os.path.expanduser("~/.mangaba")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MANGABA_HOME", Path.home() / ".mangaba")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Mangaba config paths
# ═══════════════════════════════════════════════════════════════════

MANGABA_HOME = Path(os.getenv("MANGABA_HOME", Path.home() / ".mangaba"))
CONFIG_PATH = MANGABA_HOME / "config.yaml"
PREFILL_PATH = MANGABA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Mangaba models — already uncensored, just needs clean prompt
    "mangaba": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "mangaba" in model_lower or "nous" in model_lower:
        return "mangaba"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Mangaba config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.mangaba/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Mangaba for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Mangaba for changes to take effect.")
