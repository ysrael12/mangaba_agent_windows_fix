"""
Microsoft Teams platform adapter for Mangaba Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Mangaba"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        mangaba_action = data.get("mangaba_action", "")
        session_key = data.get("session_key", "")

        if not mangaba_action or not session_key:
        ...
        choice = choice_map.get(mangaba_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="mangaba_approve",
                    data={**btn_data_base, "mangaba_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="mangaba_approve",
                    data={**btn_data_base, "mangaba_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="mangaba_approve",
                    data={**btn_data_base, "mangaba_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="mangaba_approve",
                    data={**btn_data_base, "mangaba_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Mangaba\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.mangaba/.env")
