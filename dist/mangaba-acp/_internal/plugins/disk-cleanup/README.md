...
Auto-tracks and cleans up ephemeral files created during Mangaba Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MANGABA_HOME` and `/tmp/mangaba-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MANGABA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MANGABA_HOME` or `/tmp/mangaba-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MANGABA_HOME/disk-cleanup/` is itself excluded
- `$MANGABA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
