...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/mangaba>
...
```bash
mangaba honcho setup    # full interactive wizard (cloud or local)
mangaba memory setup    # generic picker, also works
```

Or manually:
```bash
mangaba config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MANGABA_HOME/honcho.json` | Profile-local (isolated Mangaba instances) |
| 2 | `~/.mangaba/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Mangaba profile: `mangaba` (default) or `mangaba.<profile>`.
...
| 4 | `per-session` strategy | Mangaba session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`mangaba-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`mangaba`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-mangaba-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening mangaba in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Mangaba session ID (timestamp + hex). Every `mangaba` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Mangaba profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`mangaba`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Mangaba profile: `mangaba` (default) or `mangaba.<profile>` (e.g. `mangaba -p coder` → host key `mangaba.coder`).
...
| `MANGABA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `mangaba honcho setup` | Full interactive setup wizard |
| `mangaba honcho status` | Show resolved config for active profile |
| `mangaba honcho enable` / `disable` | Toggle Honcho for active profile |
| `mangaba honcho mode <mode>` | Change recall or observation mode |
| `mangaba honcho peer --user <name>` | Update user peer name |
| `mangaba honcho peer --ai <name>` | Update AI peer name |
| `mangaba honcho tokens --context <N>` | Set context token budget |
| `mangaba honcho tokens --dialectic <N>` | Set dialectic max chars |
| `mangaba honcho map <name>` | Map current directory to a session name |
| `mangaba honcho sync` | Create host blocks for all Mangaba profiles |
