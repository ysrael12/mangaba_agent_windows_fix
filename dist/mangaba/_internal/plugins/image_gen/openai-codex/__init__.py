...
Output is saved as PNG under ``$MANGABA_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `mangaba auth codex` (or `mangaba setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`mangaba auth codex` (or `mangaba setup` → Codex) to sign in."
                ),
