...
Usage (via cron with --no-agent):
    ...
    mangaba cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MANGABA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Mangaba-Watcher/1.0"})
