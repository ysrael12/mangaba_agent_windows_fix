...
## 5. Mangaba Configuration

### Config location

`$MANGABA_HOME/config.yaml` (defaults to `~/.mangaba/config.yaml` when `MANGABA_HOME` is unset)
...
Restart the Mangaba session for changes to take effect. The MCP connection is
