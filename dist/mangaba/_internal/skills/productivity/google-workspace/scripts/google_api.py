#!/usr/bin/env python3
"""Google Workspace API CLI for Mangaba Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Mangaba-facing JSON contract and falls back to the Python client
...
from _mangaba_home import get_mangaba_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MANGABA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Mangaba Agent")
