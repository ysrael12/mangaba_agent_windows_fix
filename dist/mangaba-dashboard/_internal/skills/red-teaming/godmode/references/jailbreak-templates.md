...
## 5. GODMODE FAST — Mangaba 4 405B (Zero Refusal)

**Model:** `dheiver2/mangaba-4-405b`
**Strategy:** Mangaba models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Mangaba Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.mangaba/config.yaml`:
