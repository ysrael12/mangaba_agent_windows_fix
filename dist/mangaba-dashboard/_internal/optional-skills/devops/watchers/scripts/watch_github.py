...
Usage (via cron with --no-agent):
    ...
    mangaba cron create mangaba-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$MANGABA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name mangaba-issues --repo dheiver2/mangaba-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.mangaba/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Mangaba-Watcher/1.0",
    }
