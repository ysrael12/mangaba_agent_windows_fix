"""SimpleX Chat platform adapter (Mangaba plugin).
...
This adapter ships as a Mangaba platform plugin under
``plugins/platforms/simplex/``. The Mangaba plugin loader scans the
...
discoverable and `mangaba setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Mangaba — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"mangaba-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``mangaba setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.mangaba/.env`` via ``mangaba_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("mangaba_cli.config not available; set SIMPLEX_* vars manually in ~/.mangaba/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `mangaba gateway status` without
