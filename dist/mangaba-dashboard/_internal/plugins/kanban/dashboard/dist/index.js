/**
 * Mangaba Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.mangaba/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__MANGABA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__MANGABA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "mangaba-kanban-dot-triage",
    todo: "mangaba-kanban-dot-todo",
    ready: "mangaba-kanban-dot-ready",
    running: "mangaba-kanban-dot-running",
    blocked: "mangaba-kanban-dot-blocked",
    done: "mangaba-kanban-dot-done",
    archived: "mangaba-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-mangaba-task";
  ...
  const DOCS_URL = "https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://dheiver2.github.io/mangaba-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "mangaba.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="mangaba-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "mangaba-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "mangaba-kanban-md",
  ...
  // standard `drop` event and our `mangaba-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("mangaba-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("mangaba-kanban-column--drop");
          if (target) target.classList.add("mangaba-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("mangaba-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("mangaba-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("mangaba-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "mangaba-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "mangaba-kanban-attention",
        "mangaba-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "mangaba-kanban-attention-bar" },
        h("span", { className: "mangaba-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "mangaba-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "mangaba-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "mangaba-kanban-attention-row",
                  "mangaba-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "mangaba-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "mangaba-kanban-attention-row-id" }, task.id),
                h("span", { className: "mangaba-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "mangaba-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "mangaba-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "mangaba-kanban-diag-action-btn",
      action.suggested ? "mangaba-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " mangaba-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".mangaba-kanban-drawer-comment-row input, .mangaba-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "mangaba-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("mangaba-kanban-diag", sevClass) },
      h("div", { className: "mangaba-kanban-diag-header" },
        h("span", { className: "mangaba-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "mangaba-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "mangaba-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "mangaba-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "mangaba-kanban-diag-data-row" },
                  h("span", { className: "mangaba-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "mangaba-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "mangaba-kanban-diag-data-row" },
                h("span", { className: "mangaba-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "mangaba-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "mangaba-kanban-diag-reassign-row" },
            h("span", { className: "mangaba-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "mangaba-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "mangaba-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "mangaba-kanban-diag-msg",
              msg.ok ? "mangaba-kanban-diag-msg--ok" : "mangaba-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "mangaba-kanban-section" },
      h("div", { className: "mangaba-kanban-section-head-row" },
        h("span", { className: "mangaba-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "mangaba-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "mangaba-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "mangaba-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "mangaba-kanban-docs-link",
      title: "Open Mangaba Kanban docs in a new tab",
      "aria-label": "Mangaba Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `mangaba kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "mangaba-kanban-msg-ok" : "mangaba-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "mangaba-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "mangaba-kanban-boardswitcher" },
      h("div", { className: "mangaba-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "mangaba-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "mangaba-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "mangaba-kanban-dialog-title" },
        ...
        h("div", { className: "mangaba-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Mangaba profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "mangaba-kanban-bulk" },
      h("span", { className: "mangaba-kanban-bulk-count" },
      ...
      h("div", { className: "mangaba-kanban-bulk-priority",
      ...
      h("div", { className: "mangaba-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Mangaba profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "mangaba-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("mangaba-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("mangaba-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "mangaba-kanban-trash",
        dragOver ? "mangaba-kanban-trash--drop" : "",
        props.draggingTaskId ? "mangaba-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "mangaba-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "mangaba-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".mangaba-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "mangaba-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("mangaba-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("mangaba-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "mangaba-kanban-column",
        dragOver ? "mangaba-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "mangaba-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "mangaba-kanban-col-check",
        ...
        h("span", { className: cn("mangaba-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "mangaba-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "mangaba-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "mangaba-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "mangaba-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "mangaba-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "mangaba-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "mangaba-kanban-lane" },
                  h("div", { className: "mangaba-kanban-lane-head" },
                    h("span", { className: "mangaba-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "mangaba-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "mangaba-kanban-card--stale-red";
    if (age >= tier.amber) return "mangaba-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".mangaba-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "mangaba-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "mangaba-kanban-card",
        props.selected ? "mangaba-kanban-card--selected" : "",
        props.failed ? "mangaba-kanban-card--failed" : "",
        props.draggingSource ? "mangaba-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "mangaba-kanban-card-content" },
          h("div", { className: "mangaba-kanban-card-row" },
            h("label", {
              className: "mangaba-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "mangaba-kanban-card-check",
            ...
            h("span", { className: "mangaba-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or mangaba kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "mangaba-kanban-warning-badge",
                    "mangaba-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "mangaba-kanban-needs-assignee",
          ...
          h("div", { className: "mangaba-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "mangaba-kanban-card-row mangaba-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "mangaba-kanban-assignee",
                            title: `Assigned to Mangaba profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "mangaba-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "mangaba-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "mangaba-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "mangaba-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Mangaba profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Mangaba profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "mangaba-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "mangaba-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "mangaba-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "mangaba-kanban-drawer-close",
        ...
        data ? h("div", { className: "mangaba-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "mangaba-kanban-drawer-body" },
      h("div", { className: "mangaba-kanban-drawer-title" },
        h("span", { className: cn("mangaba-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "mangaba-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "mangaba-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "mangaba-kanban-section" },
        h("div", { className: "mangaba-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "mangaba-kanban-section" },
        h("div", { className: "mangaba-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "mangaba-kanban-comment" },
                h("div", { className: "mangaba-kanban-comment-head" },
                  h("span", { className: "mangaba-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "mangaba-kanban-comment-ago" },
      ...
      h("div", { className: "mangaba-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "mangaba-kanban-event",
              isDiag ? "mangaba-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "mangaba-kanban-event-header" },
                  h("span", { className: "mangaba-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "mangaba-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "mangaba-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "mangaba-kanban-event-header-plain" },
                  h("span", { className: "mangaba-kanban-event-kind" }, e.kind),
                  h("span", { className: "mangaba-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "mangaba-kanban-event-phantom-row" },
                  h("span", { className: "mangaba-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "mangaba-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "mangaba-kanban-event-payload" },
    ...
    return h("div", { className: "mangaba-kanban-section" },
      h("div", { className: "mangaba-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "mangaba-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `mangaba-kanban-run--${r.outcome || r.status || "ended"}`
          : "mangaba-kanban-run--active";
        return h("div", { key: r.id, className: cn("mangaba-kanban-run", outcomeClass) },
          h("div", { className: "mangaba-kanban-run-head" },
            h("span", { className: "mangaba-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "mangaba-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "mangaba-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "mangaba-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "mangaba-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "mangaba-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "mangaba-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "mangaba-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "mangaba-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "mangaba-kanban-pre mangaba-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "mangaba-kanban-meta-row" },
      h("span", { className: "mangaba-kanban-meta-label" }, props.label),
      h("span", { className: "mangaba-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "mangaba-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "mangaba-kanban-meta-row" },
        h("span", { className: "mangaba-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "mangaba-kanban-meta-value mangaba-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "mangaba-kanban-meta-row" },
        h("span", { className: "mangaba-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "mangaba-kanban-meta-value mangaba-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "mangaba-kanban-section" },
      h("div", { className: "mangaba-kanban-section-head-row" },
        h("span", { className: "mangaba-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "mangaba-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "mangaba-kanban-section" },
      h("div", { className: "mangaba-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "mangaba-kanban-deps-row" },
        h("span", { className: "mangaba-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "mangaba-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "mangaba-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "mangaba-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "mangaba-kanban-dep-chip-x",
      ...
      h("div", { className: "mangaba-kanban-deps-row" },
        h("span", { className: "mangaba-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "mangaba-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "mangaba-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "mangaba-kanban-msg-ok"
          : "mangaba-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "mangaba-kanban-section" },
      ...
      h("div", { className: "mangaba-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "mangaba-kanban-home-sub mangaba-kanban-home-sub--on"
              : "mangaba-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__MANGABA_PLUGINS__ && typeof window.__MANGABA_PLUGINS__.register === "function") {
    window.__MANGABA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
