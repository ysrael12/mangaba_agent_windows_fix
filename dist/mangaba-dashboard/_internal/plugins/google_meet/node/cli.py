"""`mangaba meet node ...` subcommand tree.

Wired into the existing ``mangaba meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``mangaba meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="mangaba-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``mangaba meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             mangaba meet node approve <name> ws://<host>:{args.port} {token}")
        try:
