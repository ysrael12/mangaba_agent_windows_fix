...
load_mangaba_dotenv(
    mangaba_home=_mangaba_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.mangaba/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_mangaba_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("MANGABA_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("MANGABA_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent MangabaCLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `mangaba` shows the prompt before constructing AIAgent; the TUI used
...
def _save_cfg(cfg: dict):
    ...
    path = _mangaba_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["MANGABA_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from mangaba_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("MANGABA_MODEL", "")
        or os.environ.get("MANGABA_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("MANGABA_TUI_PROVIDER", "").strip()
    ...
    try:
        from mangaba_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("MANGABA_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("MANGABA_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("MANGABA_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] MANGABA_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from mangaba_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown MANGABA_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in MANGABA_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid MANGABA_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid MANGABA_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from mangaba_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["MANGABA_MODEL"] = result.new_model
    os.environ["MANGABA_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # MANGABA_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["MANGABA_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["MANGABA_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from mangaba_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from mangaba_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from mangaba_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from mangaba_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("MANGABA_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("MANGABA_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("MANGABA_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("MANGABA_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("MANGABA_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("MANGABA_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `MANGABA_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"mangaba_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $MANGABA_HOME/spawn-trees/<session_id>/<timestamp>.json
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from mangaba_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from mangaba_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: mangaba_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from mangaba_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _mangaba_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from mangaba_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from mangaba_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.mangaba/.env`` into the gateway process via
    ``mangaba_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `mangaba` is interactive — use `/mangaba chat -q …` or run `mangaba` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`mangaba setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`mangaba gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`mangaba sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`mangaba config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m mangaba_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "mangaba_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from mangaba_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from mangaba_cli.config import is_managed, save_env_value
        from mangaba_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `mangaba model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.mangaba/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from mangaba_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from mangaba_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("MANGABA_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("MANGABA_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["MANGABA_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from mangaba_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["MANGABA_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from mangaba_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from mangaba_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from mangaba_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from mangaba_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("MANGABA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("MANGABA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_mangaba_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from mangaba_cli.skills_hub import do_install
        ...
        if action == "browse":
            from mangaba_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from mangaba_cli.skills_hub import inspect_skill
