"""Persistent slash-command worker — one MangabaCLI per TUI session.
...
def _run(cli: MangabaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["MANGABA_SESSION_KEY"] = args.session_key
    os.environ["MANGABA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = MangabaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
