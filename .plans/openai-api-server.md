# OpenAI-Compatible API Server for Mangaba Agent
...
SSE streaming. By exposing this endpoint, mangaba-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  mangaba-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.mangaba/.env`
2. Run `mangaba gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with mangaba-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns mangaba-agent as a model) |
...
Authorization: Bearer mangaba-api-key-here
...
But mangaba-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "mangaba-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured mangaba-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of mangaba-agent's core prompt
...
Once implemented, mangaba-agent works as a drop-in backend for:
