# Mangaba Agent Security Policy

This document describes Mangaba Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/dheiver2/mangaba-agent/security/advisories/new)
or **security@dheiver2.com**. Do not open public issues for
security vulnerabilities. **Mangaba Agent does not operate a bug
bounty program.**
...
- Environment details (`mangaba version`, commit SHA, OS, Python
  version).
...
Mangaba Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Mangaba Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Mangaba Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Mangaba Agent's documentation
...
Mangaba Agent supports two OS-level isolation postures. They address
...
Mangaba Agent supports this in two ways:

- **Mangaba Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Mangaba Agent's in-process heuristics
...
Mangaba Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Mangaba Agent
itself. Bugs in Mangaba Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Mangaba Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Mangaba Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Mangaba Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  mangaba-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Mangaba Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Mangaba Agent. Bugs in Mangaba
...
- Mangaba Agent includes supply-chain guards for MCP server
